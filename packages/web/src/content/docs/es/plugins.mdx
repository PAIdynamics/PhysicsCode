---
title: Complementos
description: Escriba sus propios complementos para extender PhysicsCode.
---

Los complementos le permiten extender PhysicsCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de PhysicsCode.
...
- `.physicscode/plugins/` - Complementos a nivel de proyecto
- `~/.config/physicscode/plugins/` - Complementos globales
...
{
  "plugin": ["physicscode-helicone-session", "physicscode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/physicscode/node_modules/`.
...
1. Configuración global (`~/.config/physicscode/physicscode.json`)
2. Configuración del proyecto (`physicscode.json`)
3. Directorio global de complementos (`~/.config/physicscode/plugins/`)
4. Directorio de complementos del proyecto (`.physicscode/plugins/`)
...
```json title=".physicscode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
PhysicsCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".physicscode/plugins/my-plugin.ts"
...
```js title=".physicscode/plugins/example.js"
...
- `client`: Un cliente SDK physicscode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar physicscode.
...
```js title=".physicscode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "physicscode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio PhysicsCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que physicscode lea archivos `.env`:

```javascript title=".physicscode/plugins/env-protection.js"
...
```javascript title=".physicscode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a physicscode:

```ts title=".physicscode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@physicscode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que physicscode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para physicscode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://physicscode.ai/docs/sdk) para obtener más detalles.
...
```ts title=".physicscode/plugins/compaction.ts"
import type { Plugin } from "@physicscode-ai/plugin"
...
```ts title=".physicscode/plugins/custom-compaction.ts"
import type { Plugin } from "@physicscode-ai/plugin"
