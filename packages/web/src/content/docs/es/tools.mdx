...
Las herramientas permiten que LLM realice acciones en su código base. PhysicsCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en PhysicsCode.
...
:::note
Esta herramienta solo está disponible cuando `PHYSICSCODE_EXPERIMENTAL_LSP_TOOL=true` (o `PHYSICSCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor PhysicsCode o cuando la variable de entorno `PHYSICSCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar PhysicsCode:

```bash
PHYSICSCODE_ENABLE_EXA=1 physicscode
```
