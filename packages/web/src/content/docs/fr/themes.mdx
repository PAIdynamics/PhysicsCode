...
Avec PhysicsCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, PhysicsCode utilise notre propre thème `physicscode`.
...
PhysicsCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que PhysicsCode corresponde à l'apparence de leur terminal
...
PhysicsCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/physicscode/themes/*.json` ou `$XDG_CONFIG_HOME/physicscode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.physicscode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.physicscode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/physicscode/themes
vim ~/.config/physicscode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .physicscode/themes
vim .physicscode/themes/my-theme.json
```
...
{
  "$schema": "https://physicscode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
