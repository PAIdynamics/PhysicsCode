...
physicscode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak physicscode kendi `physicscode` temasını kullanır.
...
physicscode birden fazla yerleşik temayla gelir.
...
- physicscode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
physicscode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/physicscode/themes/*.json` veya `$XDG_CONFIG_HOME/physicscode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.physicscode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.physicscode/themes/*.json`
