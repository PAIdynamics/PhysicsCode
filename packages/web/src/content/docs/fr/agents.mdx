...
Il existe deux types d'agents dans PhysicsCode ; agents primaires et sous-agents.
...
PhysicsCode est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
PhysicsCode est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

PhysicsCode est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `physicscode.json` :
...
- Global : `~/.config/physicscode/agents/`
- Par projet : `.physicscode/agents/`

```markdown title="~/.config/physicscode/agents/review.md"
---
...
Si aucune température n'est spécifiée, PhysicsCode utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale PhysicsCode et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration PhysicsCode utilise le format `provider/model-id`. Par exemple, si vous utilisez [PhysicsCode Zen](/docs/zen), vous utiliserez `physicscode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="physicscode.json" {3-6,9-12}
{
  "$schema": "https://physicscode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="physicscode.json" {3-5,8-10}
{
  "$schema": "https://physicscode.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="physicscode.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Exécutez `physicscode models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/PAIdynamics/PhysicsCode).
:::
...
```markdown title="~/.config/physicscode/agents/docs-writer.md"
---
...
```markdown title="~/.config/physicscode/agents/security-auditor.md"
---
