---
title: Plugins
description: Skriv dine egne plugins for at udvide PhysicsCode.
---

Plugins giver dig mulighed for at udvide PhysicsCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre PhysicsCodes standardadfærd.
...
- `.physicscode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/physicscode/node_modules/`.
...
1. Global konfiguration (`~/.config/physicscode/physicscode.json`)
2. Projektkonfiguration (`physicscode.json`)
3. Global plugin-mappe (`~/.config/physicscode/plugins/`)
4. Projekt plugin bibliotek (`.physicscode/plugins/`)
...
PhysicsCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En physicscode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide physicscode.
...
:::note
Hvis du bruger PhysicsCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå physicscode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til physicscode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som physicscode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for physicscode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://physicscode.ai/docs/sdk) for detaljer.
