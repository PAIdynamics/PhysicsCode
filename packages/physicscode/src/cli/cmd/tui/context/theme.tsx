...
import physicscode from "./theme/physicscode.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@physicscode-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "physicscode")
        draft.active = typeof active === "string" ? active : "physicscode"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.physicscode, store.mode)
    })
