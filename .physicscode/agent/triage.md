...
mode: primary
hidden: true
model: physicscode/minimax-m2.5
color: "#44BA81"
tools:
  "*": false
  "github-triage": true
...
- `physicscode web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `physicscode web` view. **Do not** add for terminal, TUI, or general physicscode issues.
...
**Only** add if the issue mentions "zen" or "physicscode zen" or "physicscode black".

If the issue doesn't have "zen" or "physicscode black" in it then don't add zen label

#### core

Use for core server issues in `packages/physicscode/`, excluding `packages/physicscode/src/cli/cmd/tui/`.
...
TUI (`packages/physicscode/src/cli/cmd/tui/...`):
...
Core (`packages/physicscode/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for physicscode server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
