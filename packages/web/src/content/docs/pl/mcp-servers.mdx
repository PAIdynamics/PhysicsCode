...
Możesz dodać dodatkowe informacje do physicscode za pomocą _Model Context Protocol_, w skrócie MCP. physicscode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [physicscode Config](https://physicscode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/physicscode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
physicscode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, physicscode:
...
Jeśli serwer wymaga uwierzytelnienia, physicscode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `physicscode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji physicscode bezpieczne przechowa tokeny w `~/.local/share/physicscode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w physicscode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie physicscode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć physicscode z kontem Sentry.
