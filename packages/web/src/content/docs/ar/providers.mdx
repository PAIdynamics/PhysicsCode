---
title: المزوّدون
description: استخدام أي موفّر LLM في PhysicsCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم PhysicsCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات PhysicsCode.
...
في `~/.local/share/physicscode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات PhysicsCode.
...
PhysicsCode Zen هي قائمة نماذج يوفّرها فريق PhysicsCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع PhysicsCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع PhysicsCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر physicscode، ثم انتقل إلى [physicscode.ai/auth](https://physicscode.ai/auth).
...
يعمل مثل أي مزوّد آخر في PhysicsCode واستخدامه اختياري بالكامل.

---

## PhysicsCode Go

PhysicsCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق PhysicsCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع PhysicsCode.

1. شغّل الأمر `/connect` في TUI، واختر `PhysicsCode Go`، ثم انتقل إلى [physicscode.ai/auth](https://physicscode.ai/zen).
...
لاستخدام Amazon Bedrock مع PhysicsCode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل physicscode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `physicscode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل PhysicsCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين physicscode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل physicscode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات physicscode.
...
تخزين الرمز في مساحة تخزين مصادقة physicscode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم PhysicsCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر PhysicsCode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`physicscode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [GitLab authentication helper](https://www.npmjs.com/package/opencode-gitlab-auth).

##### التهيئة

خصّص عبر `physicscode.json`:
...
لاستخدام اشتراك GitHub Copilot مع physicscode:
...
لاستخدام Google Vertex AI مع PhysicsCode:
...
2. عيّن متغيرات البيئة المطلوبة:
   ...
   عيّنها أثناء تشغيل physicscode.
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر physicscode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [physicscode-helicone-session](https://github.com/H2Shami/physicscode-helicone-session) لتسجيل كل محادثة PhysicsCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة PhysicsCode مدرجة كجلسة مستقلة.
...
يمكنك ضبط physicscode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط physicscode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط physicscode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع PhysicsCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/physicscode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع PhysicsCode:
...
3. انسخ مفتاح API لاستخدامه في PhysicsCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في PhysicsCode، يجب سحب معلومات النموذج محليا:
...
PhysicsCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق PhysicsCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>PhysicsCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **PhysicsCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ PhysicsCode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات physicscode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات physicscode. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات physicscode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع physicscode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `physicscode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ PhysicsCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `physicscode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات physicscode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات physicscode.
