...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run physicscode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for physicscode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found physicscode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["physicscode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting physicscode...")
        UI.println()

        const physicscodeArgs = sessionId ? ["-s", sessionId] : []
        const physicscodeProcess = Process.spawn(["physicscode", ...physicscodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await physicscodeProcess.exited
        if (code !== 0) throw new Error(`physicscode exited with code ${code}`)
      },
    })
  },
})
