...
Agentferdigheter lar PhysicsCode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
PhysicsCode søker etter disse stedene:

- Prosjektkonfigurasjon: `.physicscode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/physicscode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går PhysicsCode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.physicscode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/physicscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.physicscode/skills/git-release/SKILL.md` slik:
...
PhysicsCode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `physicscode.json`:
...
**For innebygde agenter** (i `physicscode.json`):
