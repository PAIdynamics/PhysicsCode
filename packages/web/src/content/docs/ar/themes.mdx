...
مع PhysicsCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم PhysicsCode سمتنا `physicscode`.
...
يأتي PhysicsCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق PhysicsCode مظهر terminal لديهم
...
يدعم PhysicsCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/physicscode/themes/*.json` أو `$XDG_CONFIG_HOME/physicscode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.physicscode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.physicscode/themes/*.json`
