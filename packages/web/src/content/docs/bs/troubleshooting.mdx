...
Da biste otklonili probleme s PhysicsCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\physicscode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `physicscode --log-level DEBUG`.

---

## Pohrana

PhysicsCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/physicscode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\physicscode`
...
PhysicsCode Desktop pokreće lokalni PhysicsCode server (`physicscode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `PhysicsCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/physicscode/physicscode.jsonc` (ili `~/.config/physicscode/physicscode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/physicscode/physicscode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\physicscode\physicscode.jsonc`
...
PhysicsCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/physicscode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\physicscode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga PhysicsCode mogao ponovo izgraditi.

1. Potpuno zatvorite PhysicsCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/physicscode`
- **Linux**: obrišite `~/.cache/physicscode` (ili pokrenite `rm -rf ~/.cache/physicscode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\physicscode`

3. Ponovo pokrenite PhysicsCode Desktop.

---

### Rješavanje problema sa vezom na serveru

PhysicsCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `physicscode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `PHYSICSCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `PHYSICSCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, PhysicsCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa PhysicsCode karakteristikama.

---

### Obavještenja se ne prikazuju

PhysicsCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za PhysicsCode u postavkama vašeg OS-a, i
...
1. Zatvorite PhysicsCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka PhysicsCode Desktop aplikacije):

- `physicscode.settings.dat` (URL zadanog servera za desktop)
- `physicscode.global.dat` i `physicscode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s PhysicsCode:
...
Da saznate kojim modelima imate pristup, pokrenite `physicscode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\physicscode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. PhysicsCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\physicscode`

2. Ponovo pokrenite PhysicsCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti PhysicsCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
PhysicsCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
