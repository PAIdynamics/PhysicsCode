...
Las habilidades del agente permiten a PhysicsCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
PhysicsCode busca estas ubicaciones:

- Configuración del proyecto: `.physicscode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/physicscode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, PhysicsCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.physicscode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/physicscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.physicscode/skills/git-release/SKILL.md` así:
...
PhysicsCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `physicscode.json`:
...
**Para agentes integrados** (en `physicscode.json`):
