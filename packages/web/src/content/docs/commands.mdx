...
Create `.physicscode/commands/test.md`:
...
You can add custom commands through the PhysicsCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your PhysicsCode [config](/docs/config):

```json title="physicscode.jsonc" {3-11}
{
  "command": {
...
- Per-project: `.physicscode/commands/`
...
physicscode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
