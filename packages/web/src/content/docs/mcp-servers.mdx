...
You can add external tools to PhysicsCode using the _Model Context Protocol_, or MCP. PhysicsCode supports both local and remote servers.
...
You can define MCP servers in your [PhysicsCode Config](/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="physicscode.jsonc" {5}
{
  "mcp": {
    "name-of-mcp-server": {
      // ...
...
Organizations can provide default MCP servers via their `.well-known/physicscode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
```jsonc title="physicscode.jsonc" {14}
{
  "mcp": {
    "my-local-mcp-server": {
      "type": "local",
...
PhysicsCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, PhysicsCode will:
...
If the server requires authentication, PhysicsCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `physicscode mcp auth <server-name>`.
...
```json title="physicscode.json" {6-10}
{
  "mcp": {
    "my-oauth-server": {
      "type": "remote",
...
```bash
physicscode mcp auth my-oauth-server
```
...
```bash
physicscode mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, PhysicsCode will store the tokens securely in `~/.local/share/physicscode/mcp-auth.json`.
...
physicscode mcp debug my-oauth-server
```
...
Your MCPs are available as tools in PhysicsCode, alongside built-in tools. So you can manage them through the PhysicsCode config like any other tool.
...
```json title="physicscode.json" {13}
{
  "mcp": {
    "my-mcp-foo": {
      "type": "local",
...
```json title="physicscode.json" {10,13-17}
{
  "mcp": {
    "my-mcp": {
      "type": "local",
...
PhysicsCode Science is a provenance-aware retrieval service for scientific
software — source-backed evidence from an indexed set of scientific
repositories (solvers, simulation libraries, HPC codes), used by the `/science`
command and `science` agent.

**In PhysicsCode itself**, this is already configured automatically once
you're logged in (`physicscode account login`) — no setup needed, and no
entry needed in your `mcp` config.

**In Claude Code or Codex**, add it manually, authenticated with a PhysicsCode
API key (create one from the account portal's Profile tab, or run
`physicscode account login` and reuse that session's token). This is the
hosted endpoint, so it works from any machine with the key — no local server
or install required:

```bash
claude mcp add --transport http physicscode-science https://www.physicscode.ai/mcp \
  --header "Authorization: Bearer <your PhysicsCode API key>"
```

```bash
export PHYSICSCODE_TOKEN=<your PhysicsCode API key>
codex mcp add physicscode-science \
  --url https://www.physicscode.ai/mcp \
  --bearer-token-env-var PHYSICSCODE_TOKEN
```

This gives either client the `science_search`, `science_get_source`,
`science_get_symbol`, `science_get_context`, `science_check_license`,
`science_project_context`, and `science_status` tools. If you're running your
own local science index instead of using the hosted one, see
[`physicscode-science/docs/mcp-clients.md`](https://github.com/PAIdynamics/PhysicsCode/blob/main/physicscode-science/docs/mcp-clients.md)
for the stdio and local-HTTP transport options.
...
```json title="physicscode.json" {3-7}
{
  "mcp": {
    "sentry": {
      "type": "remote",
...
```bash
physicscode mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect PhysicsCode to your Sentry account.
...
```json title="physicscode.json" {3-6}
{
  "mcp": {
    "context7": {
      "type": "remote",
...
```json title="physicscode.json" {6-8}
{
  "mcp": {
    "context7": {
      "type": "remote",
