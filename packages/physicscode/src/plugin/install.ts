...
export type PatchDeps = {
  ...
  files: (dir: string, name: "physicscode" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".physicscode")
}

function patchName(kind: Kind): "physicscode" | "tui" {
  if (kind === "server") return "physicscode"
  return "tui"
}
