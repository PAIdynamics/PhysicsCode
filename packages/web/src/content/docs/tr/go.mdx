...
PhysicsCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
PhysicsCode Go şu anda beta olarak mevcut.
:::

Go, PhysicsCode'daki diğer sağlayıcılar gibi çalışır. PhysicsCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve PhysicsCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
PhysicsCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

PhysicsCode Go, PhysicsCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>PhysicsCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `PhysicsCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye PhysicsCode Go'ya abone olabilir.
:::
...
PhysicsCode Go aşağıdaki limitleri içerir:
...
PhysicsCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `physicscode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `physicscode-go/kimi-k2.6` kullanmalısınız.
...
PhysicsCode Go'yu şu amaçlarla oluşturduk:
...
4. PhysicsCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
