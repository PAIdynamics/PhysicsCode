...
fn configure_display_backend() -> Option<String> {
    use physicscode_lib::linux_windowing::{Backend, SessionEnv, select_backend};
    use std::env;
    ...
    // allocation with EACCES, leaving the window blank. This is independent of
    // Wayland-vs-X11 backend selection below, so it must run unconditionally
    // rather than inside the `select_backend` decision (which returns None for
    // plain X11 sessions and would otherwise skip this entirely).
