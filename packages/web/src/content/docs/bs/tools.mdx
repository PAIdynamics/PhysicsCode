...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. PhysicsCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u PhysicsCode.
...
:::note
Ovaj alat je dostupan samo kada je `PHYSICSCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `PHYSICSCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz PhysicsCode provajdera ili kada je varijabla `PHYSICSCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju PhysicsCode:
