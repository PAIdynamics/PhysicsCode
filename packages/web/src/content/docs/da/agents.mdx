...
Der er to typer agenter i PhysicsCode; primære agenter og subagenter.
...
PhysicsCode leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
PhysicsCode leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

PhysicsCode leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Konfigurer agent i din `physicscode.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger PhysicsCode modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale PhysicsCode-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din PhysicsCode-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [PhysicsCode Zen](/docs/zen), vil du bruge `physicscode/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `physicscode models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/PAIdynamics/PhysicsCode).
:::
