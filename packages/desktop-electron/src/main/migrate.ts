...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.physicscode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `physicscode.settings.dat` is special: it maps to the `physicscode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"physicscode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // physicscode.settings.dat → the electron settings store ("physicscode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "physicscode.global.dat").
  const storeName = filename === "physicscode.settings.dat" ? "physicscode.settings" : filename
