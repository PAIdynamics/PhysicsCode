---
title: Regeln
description: Lege eigene Anweisungen fuer physicscode fest.
---

Du kannst physicscode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in physicscode den Befehl `/init` aus.
...
So kann physicscode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht physicscode sie zu erweitern.
...
physicscode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/physicscode/AGENTS.md` speichern.
Diese gelten in allen physicscode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt PhysicsCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/physicscode/AGENTS.md` existiert)
...
Beim Start sucht physicscode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/physicscode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `physicscode.json` oder in `~/.config/physicscode/physicscode.json` angeben.
...
physicscode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit physicscode.json

Empfohlen ist das `instructions`-Feld in der `physicscode.json`:
...
Du kannst physicscode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `physicscode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
