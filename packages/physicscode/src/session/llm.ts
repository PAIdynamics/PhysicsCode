...
export class Service extends Context.Service<Service, Interface>()("@physicscode/LLM") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
      ...
      // from the workflow service are executed via physicscode's tool system
      ...
      const physicscodeProjectID = input.model.providerID.startsWith("physicscode")
      ...
      return streamText({
        ...
        abortSignal: input.abort,
        headers: {
          ...(input.model.providerID.startsWith("physicscode")
            ? {
                "x-physicscode-project": physicscodeProjectID,
                "x-physicscode-session": input.sessionID,
                "x-physicscode-request": input.user.id,
                "x-physicscode-client": Flag.PHYSICSCODE_CLIENT,
                "User-Agent": `physicscode/${InstallationVersion}`,
              }
            : {
