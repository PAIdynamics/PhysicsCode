...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@physicscode-ai/sdk/v2"
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `physicscode auth login` in the terminal",
      name: "Login with physicscode",
      id: "physicscode-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // PhysicsCode stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const physicscodeProvider = providers.find((p) => p.id === "physicscode")
  if (physicscodeProvider) {
    if (physicscodeProvider.models["big-pickle"]) {
      return { providerID: ProviderID.physicscode, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(physicscodeProvider.models))
    if (best) {
      return {
