...
description: PhysicsCode, LSP sunucularınızla bütünleşir.
---

PhysicsCode, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

PhysicsCode, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`PHYSICSCODE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
physicscode bir dosyayı açtığında:
...
LSP sunucularını, physicscode yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
