---
title: Server
description: Interager med physicscode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `physicscode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en physicscode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `physicscode.local` |
...
Indstil `PHYSICSCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `physicscode`, eller indstil `PHYSICSCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `physicscode serve` og `physicscode web`.
...
Når du kører `physicscode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug physicscode-serveren til at interagere med physicscode programmatisk.
:::

Denne arkitektur lader physicscode understøtte flere klienter og lader dig interagere med physicscode programmatisk.

Du kan køre `physicscode serve` for at starte en selvstændig server. Hvis du har
physicscode TUI kørende, vil `physicscode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af PhysicsCode [IDE](/docs/ide) plugins.
...
PhysicsCode-serveren viser følgende API'er.
