...
As habilidades do agente permitem que o physicscode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O physicscode pesquisa nesses locais:

- Configuração do projeto: `.physicscode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/physicscode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o physicscode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.physicscode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/physicscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.physicscode/skills/git-release/SKILL.md` assim:
...
O physicscode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `physicscode.json`:
...
**Para agentes embutidos** (em `physicscode.json`):
