...
Det er to typer agenter i PhysicsCode; primære agenter og underagenter.
...
PhysicsCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
PhysicsCode kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

PhysicsCode kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `physicscode.json`:
...
- Globalt: `~/.config/physicscode/agents/`
- Per prosjekt: `.physicscode/agents/`
...
Hvis ingen temperatur er spesifisert, bruker PhysicsCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale PhysicsCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i PhysicsCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [PhysicsCode Zen](/docs/zen), vil du bruke `physicscode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `physicscode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/PAIdynamics/PhysicsCode).
:::
