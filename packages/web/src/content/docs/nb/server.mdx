---
title: Tjener
description: Samhandle med PhysicsCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `physicscode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en PhysicsCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `physicscode.local` |
...
Sett `PHYSICSCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `physicscode` som standard, eller sett `PHYSICSCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `physicscode serve` og `physicscode web`.
...
Når du kjører `physicscode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk PhysicsCode-serveren til å samhandle med PhysicsCode programmatisk.
:::

Denne arkitekturen lar PhysicsCode støtte flere klienter og lar deg samhandle med PhysicsCode programmatisk.

Du kan kjøre `physicscode serve` for å starte en frittstående server. Hvis du har
PhysicsCode TUI kjører, vil `physicscode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av PhysicsCode [IDE](/docs/ide) plugins.
...
physicscode-serveren viser følgende APIer.
