---
title: TUI
description: Using the PhysicsCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

PhysicsCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running PhysicsCode starts the TUI for the current directory.
...
When using the PhysicsCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to PhysicsCode. Allows you to select from available providers and add their API keys.
...
Exit PhysicsCode. _Aliases_: `/quit`, `/q`
...
This is separate from `physicscode.json`, which configures server/runtime behavior.
...
Use `PHYSICSCODE_TUI_CONFIG` to load a custom TUI config path.
