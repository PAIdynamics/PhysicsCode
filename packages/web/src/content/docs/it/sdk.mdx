---
title: SDK
description: Client JS type-safe per il server physicscode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

L'SDK JS/TS di physicscode fornisce un client type-safe per interagire con il server.
Usalo per creare integrazioni e controllare physicscode in modo programmatico.
...
Crea un'istanza di physicscode:
...
Puoi passare un oggetto di configurazione per personalizzare il comportamento. L'istanza legge comunque `physicscode.json`, ma puoi sovrascrivere o aggiungere configurazione inline:
...
Se hai gia un'istanza di physicscode in esecuzione, puoi creare un client per collegarti:
