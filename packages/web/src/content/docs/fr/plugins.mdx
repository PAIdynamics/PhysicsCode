---
title: Plugins
description: Écrivez vos propres plugins pour étendre PhysicsCode.
---

Les plugins vous permettent d'étendre PhysicsCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de PhysicsCode.
...
- `.physicscode/plugins/` - Plugins au niveau du projet
- `~/.config/physicscode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/physicscode/node_modules/`.
...
1. Configuration globale (`~/.config/physicscode/physicscode.json`)
2. Configuration du projet (`physicscode.json`)
3. Répertoire global des plugins (`~/.config/physicscode/plugins/`)
4. Répertoire des plugins du projet (`.physicscode/plugins/`)
...
PhysicsCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK physicscode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre physicscode.
...
:::note
Si vous utilisez l'application de bureau PhysicsCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez physicscode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à physicscode :
...
L'assistant `tool` crée un outil personnalisé que physicscode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour physicscode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://physicscode.ai/docs/sdk) pour plus de détails.
