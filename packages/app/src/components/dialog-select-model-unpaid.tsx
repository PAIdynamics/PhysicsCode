            ...
            <Tooltip
              ...
              value={
                <ModelTooltip
                  model={item}
                  latest={item.latest}
                  free={item.provider.id === "physicscode" && (!item.cost || item.cost.input === 0)}
                />
              }
            >
              {node}
                ...
                {(i) => (
                  <div class="w-full flex items-center gap-x-3">
                    ...
                    <Show when={i.id === "physicscode"}>
                      <div class="text-14-regular text-text-weak">{language.t("dialog.provider.physicscode.tagline")}</div>
                    </Show>
                    <Show when={i.id === "physicscode"}>
                      <Tag>{language.t("dialog.provider.tag.recommended")}</Tag>
                    </Show>
                    <Show when={i.id === "physicscode-go"}>
                      <>
                        <div class="text-14-regular text-text-weak">
                          {language.t("dialog.provider.physicscodeGo.tagline")}
                        </div>
                        <Tag>{language.t("dialog.provider.tag.recommended")}</Tag>
                      </>
                    </Show>
