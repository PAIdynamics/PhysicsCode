---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u PhysicsCode.
---

import config from "../../../../config.mjs"
export const console = config.console

PhysicsCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj PhysicsCode konfiguraciji.
...
u `~/.local/share/physicscode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem PhysicsCode
config.
...
PhysicsCode Zen je lista modela koje je obezbedio PhysicsCode tim koji su bili
testirano i potvrđeno da dobro radi sa PhysicsCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa PhysicsCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite physicscode i idite na [physicscode.ai/auth](https://physicscode.ai/auth).
...
Radi kao i svaki drugi provajder u PhysicsCode i potpuno je opcionalan za korištenje.

---

## PhysicsCode Go

PhysicsCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža PhysicsCode tim i koji su testirani i verificirani da dobro rade s PhysicsCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `PhysicsCode Go` i idite na [physicscode.ai/auth](https://physicscode.ai/zen).
...
Da biste koristili Amazon Bedrock s PhysicsCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete physicscode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `physicscode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u PhysicsCode.
:::
...
Možete konfigurirati physicscode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi physicscode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju physicscode konfiguraciju.
...
da pohrani token u physicscode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
PhysicsCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate PhysicsCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`physicscode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [GitLab authentication helper](https://www.npmjs.com/package/opencode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `physicscode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s physicscode:
...
Za korištenje Google Vertex AI s PhysicsCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće physicscode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem physicscodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [physicscode-helicone-session](https://github.com/H2Shami/physicscode-helicone-session) da automatski prijavite svaki PhysicsCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki PhysicsCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati physicscode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati physicscode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati physicscode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za PhysicsCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/physicscode) za detalje.
:::
...
Da biste koristili Ollama Cloud s PhysicsCode:
...
3. Kopirajte API ključ za korištenje u PhysicsCode.
...
6. **Važno**: Prije upotrebe modela oblaka u PhysicsCode, morate lokalno povući informacije o modelu:
...
PhysicsCode Zen je lista testiranih i verifikovanih modela koju je obezbedio PhysicsCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>PhysicsCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **PhysicsCode Zen**.

```txt
   /connect
```

3. Unesite svoj PhysicsCode API ključ.
...
Također možete dodati dodatne modele putem vaše physicscode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše physicscode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju physicscode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s physicscode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `physicscode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju PhysicsCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `physicscode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite PhysicsCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj physicscode konfiguraciji.
