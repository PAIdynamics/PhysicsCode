---
title: Serveur
description: Interagissez avec le serveur physicscode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

La commande `physicscode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client physicscode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `physicscode.local` |
...
Définissez `PHYSICSCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `physicscode`, ou définissez `PHYSICSCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `physicscode serve` et à `physicscode web`.
...
Lorsque vous exécutez `physicscode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur physicscode pour interagir avec physicscode par programmation.
:::

Cette architecture permet à physicscode de prendre en charge plusieurs clients et vous permet d'interagir avec physicscode par programme.

Vous pouvez exécuter `physicscode serve` pour démarrer un serveur autonome. Si vous avez le
physicscode TUI en cours d'exécution, `physicscode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins PhysicsCode [IDE](/docs/ide).
...
Le serveur physicscode expose les API suivantes.
