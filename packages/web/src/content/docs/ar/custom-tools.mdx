...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في physicscode.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في physicscode مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.physicscode/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/physicscode/tools/`.
