...
const managedConfigDir = process.env.PHYSICSCODE_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "physicscode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "physicscode.json") {
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "physicscode.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "physicscode.jsonc"),
        JSON.stringify({
          $schema: "https://physicscode.ai/config.json",
          shell: "bash",
          model: "test/model",
  ...
  try {
    ...
    const file = path.join(tmp.path, "physicscode.jsonc")
...
test("ignores legacy tui keys in physicscode config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://physicscode.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "physicscode.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["PHYSICSCODE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { physicscode: { options: { apiKey: "{env:PHYSICSCODE_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["physicscode"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.provide(Npm.defaultLayer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["PHYSICSCODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["PHYSICSCODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "physicscode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .physicscode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const physicscodeDir = path.join(dir, ".physicscode")
      await fs.mkdir(physicscodeDir, { recursive: true })
      const agentDir = path.join(physicscodeDir, "agent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".physicscode", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .physicscode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const physicscodeDir = path.join(dir, ".physicscode")
      await fs.mkdir(physicscodeDir, { recursive: true })

      const agentsDir = path.join(physicscodeDir, "agents")
...
test("loads commands from .physicscode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const physicscodeDir = path.join(dir, ".physicscode")
      await fs.mkdir(physicscodeDir, { recursive: true })

      const commandDir = path.join(physicscodeDir, "command")
...
test("loads commands from .physicscode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const physicscodeDir = path.join(dir, ".physicscode")
      await fs.mkdir(physicscodeDir, { recursive: true })

      const commandsDir = path.join(physicscodeDir, "commands")
...
test("does not try to install dependencies in read-only PHYSICSCODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.PHYSICSCODE_CONFIG_DIR
  process.env.PHYSICSCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.PHYSICSCODE_CONFIG_DIR
    else process.env.PHYSICSCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable PHYSICSCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "physicscode.json"),
        JSON.stringify({ $schema: "https://physicscode.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .physicscode config
      const projectDir = path.join(dir, "project")
      const physicscodeDir = path.join(projectDir, ".physicscode")
      await fs.mkdir(physicscodeDir, { recursive: true })
      ...
      // Local .physicscode config with different plugins
      await Filesystem.write(
        path.join(physicscodeDir, "physicscode.json"),
        JSON.stringify({
          $schema: "https://physicscode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .physicscode config with some overlapping plugins
      await Filesystem.write(
        path.join(physicscodeDir, "physicscode.json"),
        JSON.stringify({
          $schema: "https://physicscode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".physicscode")
      ...
      await Filesystem.write(
        path.join(local, "physicscode.json"),
        JSON.stringify({
          $schema: "https://physicscode.ai/config.json",
...
// Note: preload.ts sets PHYSICSCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .physicscode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .physicscode directory config enables it
      const physicscodeDir = path.join(dir, ".physicscode")
      await fs.mkdir(physicscodeDir, { recursive: true })
      await Filesystem.write(
        path.join(physicscodeDir, "physicscode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/physicscode")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/physicscode")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "physicscode.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-physicscode@2.4.3", file)).toBe("oh-my-physicscode@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-physicscode@2.4.3", "file:///project/.physicscode/plugin/oh-my-physicscode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.physicscode/plugin/demo.ts", "file:///project/.physicscode/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.physicscode/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const physicscodeDir = path.join(projectDir, ".physicscode")
        const pluginDir = path.join(physicscodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("PHYSICSCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["PHYSICSCODE_DISABLE_PROJECT_CONFIG"]
    process.env["PHYSICSCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["PHYSICSCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["PHYSICSCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .physicscode/ directories when flag is set", async () => {
    const originalEnv = process.env["PHYSICSCODE_DISABLE_PROJECT_CONFIG"]
    process.env["PHYSICSCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .physicscode directory with a command
          const physicscodeDir = path.join(dir, ".physicscode", "command")
          await fs.mkdir(physicscodeDir, { recursive: true })
          await Filesystem.write(path.join(physicscodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .physicscode should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["PHYSICSCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["PHYSICSCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["PHYSICSCODE_CONFIG_DIR"]
      process.env["PHYSICSCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["PHYSICSCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["PHYSICSCODE_CONFIG_DIR"]
      } else {
        process.env["PHYSICSCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("PHYSICSCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["PHYSICSCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["PHYSICSCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["PHYSICSCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["PHYSICSCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from PHYSICSCODE_CONFIG_DIR, not project
...
describe("PHYSICSCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in PHYSICSCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["PHYSICSCODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["PHYSICSCODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://physicscode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["PHYSICSCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["PHYSICSCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in PHYSICSCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["PHYSICSCODE_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.effectSchema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "PhysicsCode Managed",
          PayloadIdentifier: "ai.physicscode.managed.test",
          PayloadType: "ai.physicscode.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://physicscode.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://physicscode.ai/config.json")
})
