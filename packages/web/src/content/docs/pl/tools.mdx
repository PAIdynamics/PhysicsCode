...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. PhysicsCode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w PhysicsCode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `PHYSICSCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `PHYSICSCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy PhysicsCode lub gdy zmienna środowiskowa `PHYSICSCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu PhysicsCode:
