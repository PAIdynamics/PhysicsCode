---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i PhysicsCode.
---

import config from "../../../../config.mjs"
export const console = config.console

PhysicsCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i PhysicsCode-konfigurasjonen.
...
i `~/.local/share/physicscode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i PhysicsCode
konfig.
...
PhysicsCode Zen er en liste over modeller levert av PhysicsCode-teamet som har vært
testet og verifisert for å fungere godt med PhysicsCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med PhysicsCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `PhysicsCode Zen` og gå til [physicscode.ai/auth](https://physicscode.ai/zen).
...
Det fungerer som alle andre leverandører i PhysicsCode og er helt valgfritt å bruke.

---

## PhysicsCode Go

PhysicsCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av PhysicsCode-teamet som har vært
testet og verifisert for å fungere godt med PhysicsCode.

1. Kjør kommandoen `/connect` i TUI, velg `PhysicsCode Go`, og gå til [physicscode.ai/auth](https://physicscode.ai/zen).
...
Slik bruker du Amazon Bedrock med PhysicsCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører PhysicsCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `physicscode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i physicscode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere physicscode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at PhysicsCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom PhysicsCode-konfigurasjonen.
...
for å lagre token i PhysicsCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
PhysicsCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse PhysicsCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`physicscode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [GitLab authentication helper](https://www.npmjs.com/package/opencode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `physicscode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med PhysicsCode:
...
Slik bruker du Google Vertex AI med PhysicsCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører PhysicsCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom PhysicsCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [physicscode-helicone-session](https://github.com/H2Shami/physicscode-helicone-session) plugin for automatisk å logge hver PhysicsCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver PhysicsCode-samtale oppført som en separat økt.
...
Du kan konfigurere PhysicsCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere PhysicsCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere PhysicsCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for PhysicsCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/physicscode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med PhysicsCode:
...
3. Kopier API-nøkkelen for bruk i PhysicsCode.
...
6. **Viktig**: Før du bruker skymodeller i PhysicsCode, må du hente modellinformasjonen lokalt:
...
PhysicsCode Zen er en liste over testede og verifiserte modeller levert av PhysicsCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>PhysicsCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **PhysicsCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn PhysicsCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom PhysicsCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom PhysicsCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med PhysicsCode:
...
Du kan også tilpasse modeller gjennom PhysicsCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med PhysicsCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `physicscode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar PhysicsCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `physicscode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk PhysicsCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i PhysicsCode-konfigurasjonen.
