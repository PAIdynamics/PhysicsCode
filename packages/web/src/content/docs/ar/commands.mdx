...
أنشئ الملف `.physicscode/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات PhysicsCode أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ PhysicsCode:
...
- على مستوى النظام: `~/.config/physicscode/commands/`
- لكل مشروع: `.physicscode/commands/`
...
يتضمن physicscode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
