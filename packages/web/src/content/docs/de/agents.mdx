...
Es gibt zwei Arten von Agenten in PhysicsCode; Primäragenten und Subagenten.
...
PhysicsCode verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `physicscode.json`-Konfigurationsdatei:
...
- Pro Projekt: `.physicscode/agents/`
...
Wenn keine Temperatur angegeben ist, verwendet PhysicsCode modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale PhysicsCode-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer PhysicsCode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [PhysicsCode Zen](/docs/zen) verwenden, würden Sie `physicscode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `physicscode models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/PAIdynamics/PhysicsCode).
:::
