---
title: TUI
description: Utilisation de l’interface utilisateur du terminal PhysicsCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

PhysicsCode fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de PhysicsCode démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le PhysicsCode TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à PhysicsCode. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez PhysicsCode. *Alias* : `/quit`, `/q`
...
Ceci est séparé de `physicscode.json`, qui configure le comportement du serveur/d'exécution.
...
Utilisez `PHYSICSCODE_TUI_CONFIG` pour charger un chemin de configuration TUI personnalisé.
