...
export class Service extends Context.Service<Service, Interface>()("@physicscode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  // Every config dir we may read from: global config dir, any `.physicscode`
  // folders between cwd and home, and PHYSICSCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.PHYSICSCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit PHYSICSCODE_TUI_CONFIG override, if set.
  if (Flag.PHYSICSCODE_TUI_CONFIG) {
    const configFile = Flag.PHYSICSCODE_TUI_CONFIG
  ...
  // 4. `.physicscode` directories (and PHYSICSCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".physicscode") || dir === Flag.PHYSICSCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".physicscode") && dir !== Flag.PHYSICSCODE_CONFIG_DIR) continue
...
async function load(text: string, configFilepath: string): Promise<Info> {
  return ConfigVariable.substitute({ text, type: "path", path: configFilepath, missing: "empty" })
    ...
    .then((data) => {
      ...
      // (mirroring the old physicscode.json shape) still get their settings applied.
