...
PhysicsCode Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

:::note
PhysicsCode Go está actualmente en beta.
:::

Go funciona como cualquier otro proveedor en PhysicsCode. Te suscribes a PhysicsCode Go y
...
PhysicsCode Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

PhysicsCode Go funciona como cualquier otro proveedor en PhysicsCode.

1. Inicias sesión en **<a href={console}>PhysicsCode Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `PhysicsCode Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a PhysicsCode Go.
:::
...
PhysicsCode Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de PhysicsCode
usa el formato `physicscode-go/<model-id>`. Por ejemplo, para Kimi K2.6, usarías
`physicscode-go/kimi-k2.6` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con PhysicsCode.
