...
import { createOpencodeClient } from "@physicscode-ai/sdk/v2"
...
function app(backend: Backend, input?: { password?: string; username?: string }) {
  Flag.PHYSICSCODE_EXPERIMENTAL_HTTPAPI = backend === "httpapi"
      ...
      Effect.gen(function* () {
        ...
        const bad = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("physicscode", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("physicscode", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
