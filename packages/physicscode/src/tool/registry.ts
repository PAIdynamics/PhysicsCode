...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@physicscode-ai/plugin"
...
import { Glob } from "@physicscode-ai/core/util/glob"
...
export class Service extends Context.Service<Service, Interface>()("@physicscode/ToolRegistry") {}
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        for (const match of matches) {
          ...
          const loaded = yield* Effect.tryPromise({
            try: () => import(pathToFileURL(match).href),
              ...
              log.warn("failed to load custom tool", { path: match, error })
              return error
            },
          }).pipe(Effect.option)
        ...
        const questionEnabled =
          ["app", "cli", "desktop"].includes(Flag.PHYSICSCODE_CLIENT) || Flag.PHYSICSCODE_ENABLE_QUESTION_TOOL
        ...
        return {
          ...
          builtin: [
            ...
            tool.skill,
            tool.patch,
            ...(Flag.PHYSICSCODE_EXPERIMENTAL_LSP_TOOL ? [tool.lsp] : []),
            ...(Flag.PHYSICSCODE_EXPERIMENTAL_PLAN_MODE && Flag.PHYSICSCODE_CLIENT === "cli" ? [tool.plan] : []),
          ],
          task: tool.task,
          read: tool.read,
        }
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      const filtered = (yield* all()).filter((tool) => {
        if (tool.id === WebSearchTool.id) {
          return input.providerID === ProviderID.physicscode || Flag.PHYSICSCODE_ENABLE_EXA
        }
