---
title: الخادم
description: تفاعل مع خادم physicscode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `physicscode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل physicscode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `physicscode.local` |
...
عيّن `PHYSICSCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `physicscode`، أو عيّن `PHYSICSCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `physicscode serve` و `physicscode web`.
...
عند تشغيل `physicscode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم physicscode للتفاعل مع physicscode برمجيا.
:::

تتيح هذه البنية لـ physicscode دعم عدة عملاء وتمكّنك من التفاعل مع physicscode برمجيا.

يمكنك تشغيل `physicscode serve` لبدء خادم مستقل. إذا كانت واجهة physicscode في terminal (TUI)
قيد التشغيل، فسيبدأ `physicscode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات PhysicsCode لـ [IDE](/docs/ide).
...
يعرض خادم physicscode واجهات API التالية.
