...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. physicscode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
physicscode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `PHYSICSCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `PHYSICSCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca physicscode provider kullanılırken veya `PHYSICSCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

physicscode başlatırken etkinleştirmek için:
