...
Создайте `.physicscode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию physicscode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем physicscode [config](/docs/config):
...
- Глобальный: `~/.config/physicscode/commands/`
- Для каждого проекта: `.physicscode/commands/`
...
physicscode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
