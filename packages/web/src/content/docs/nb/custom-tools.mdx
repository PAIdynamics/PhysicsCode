...
description: Lag verktøy som LLM kan kalle inn PhysicsCode.
---

Egendefinerte verktøy er funksjoner du oppretter som LLM kan kalle under samtaler. De fungerer sammen med PhysicsCode sine [innebygde verktøy](/docs/tools) som `read`, `write` og `bash`.
...
- Lokalt ved å plassere dem i `.physicscode/tools/`-katalogen til prosjektet ditt.
- Eller globalt, ved å plassere dem i `~/.config/physicscode/tools/`.
