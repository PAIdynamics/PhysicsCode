...
import { PhysicsCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for physicscode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw = OpenApi.fromApi(PhysicsCodeHttpApi) as unknown as OpenApiSpec
...
export const PublicApi = PhysicsCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "physicscode",
    version: "1.0.0",
    description: "physicscode api",
