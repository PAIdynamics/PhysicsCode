...
With PhysicsCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, PhysicsCode uses our own `physicscode` theme.
...
PhysicsCode comes with several built-in themes.
...
- Want PhysicsCode to match their terminal's appearance
...
PhysicsCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/physicscode/themes/*.json` or `$XDG_CONFIG_HOME/physicscode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.physicscode/themes/*.json`
4. **Current working directory** - Defined in `./.physicscode/themes/*.json`
