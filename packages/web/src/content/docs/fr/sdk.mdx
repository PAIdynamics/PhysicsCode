---
title: SDK
description: Client JS de type sécurisé pour le serveur physicscode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

Le SDK physicscode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler physicscode par programme.
...
```bash
npm install @physicscode-ai/sdk
```
...
Créez une instance de physicscode :
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `physicscode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${physicscode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de physicscode, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@physicscode-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*physicscode" },
})
