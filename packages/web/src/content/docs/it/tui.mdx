---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di PhysicsCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

PhysicsCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire PhysicsCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di PhysicsCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a PhysicsCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da PhysicsCode. _Alias_: `/quit`, `/q`
...
Questo è separato da `physicscode.json`, che configura il comportamento di server/runtime.
...
Usa `PHYSICSCODE_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
