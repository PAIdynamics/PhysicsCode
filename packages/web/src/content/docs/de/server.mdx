---
title: Server
description: Nutze den physicscode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `physicscode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein physicscode-Client nutzen kann.
...
Setze `PHYSICSCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `physicscode`, kann aber mit `PHYSICSCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `physicscode serve` und `physicscode web`.
...
Wenn du `physicscode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den physicscode-Server, um physicscode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `physicscode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `physicscode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die PhysicsCode-[IDE](/docs/ide)-Plugins.
...
Der physicscode-Server stellt folgende APIs bereit.
