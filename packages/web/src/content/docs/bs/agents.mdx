...
Postoje dvije vrste agenata u PhysicsCode: primarni agenti i podagenti.
...
PhysicsCode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
PhysicsCode dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

PhysicsCode dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `physicscode.json`:
...
- Globalno: `~/.config/physicscode/agents/`
- Po projektu: `.physicscode/agents/`
...
Ako temperatura nije navedena, PhysicsCode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu PhysicsCode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj PhysicsCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [PhysicsCode Zen](/docs/zen), koristili biste `physicscode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `physicscode models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/PAIdynamics/PhysicsCode).
:::
