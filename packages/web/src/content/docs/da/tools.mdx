...
Her er alle de innebygde verktøyene som er tilgængelige i PhysicsCode.
...
:::note
Dette verktøyet er bare tilgængelig når `PHYSICSCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `PHYSICSCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger PhysicsCode-leverandøren eller når miljøvariabelen `PHYSICSCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
