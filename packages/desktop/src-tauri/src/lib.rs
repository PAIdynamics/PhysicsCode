...
async fn initialize(app: AppHandle) {
    ...
    let _ = ready_tx.send(ServerReadyData {
        url: url.clone(),
        username: Some("physicscode".to_string()),
    ...
    let sqlite_done = needs_migration.then(|| {
        tracing::info!(
            path = %physicscode_db_path().expect("failed to get db path").display(),
...
fn get_sidecar_port() -> u32 {
    option_env!("PHYSICSCODE_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("PHYSICSCODE_PORT").ok())
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = physicscode_db_path() else {
        return true;
    };
...
fn physicscode_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("physicscode").join("physicscode.db"))
}
