...
const TUI_SCHEMA_URL = "https://physicscode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from physicscode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const physicscode = await physicscodeFiles(input)
  for (const file of physicscode) {
...
async function physicscodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "physicscode"),
    ...(await Filesystem.findUp(["physicscode.json", "physicscode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "physicscode"))
  }
  if (Flag.PHYSICSCODE_CONFIG) files.push(Flag.PHYSICSCODE_CONFIG)
