---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i PhysicsCode.
---

import config from "../../../../config.mjs"
export const console = config.console

PhysicsCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i PhysicsCode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/physicscode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i PhysicsCode-konfigurationen.
...
PhysicsCode Zen er en liste over modeller leveret af PhysicsCode-teamet, som er blevet testet og verificeret til at fungere godt med PhysicsCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med PhysicsCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg physicscode og gå til [physicscode.ai/auth](https://physicscode.ai/auth).
...
Det fungerer som alle andre udbydere i PhysicsCode og er helt valgfrit at bruge.

---

## PhysicsCode Go

PhysicsCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af PhysicsCode-teamet, som er testet og verificeret til at fungere godt med PhysicsCode.

1. Kør kommandoen `/connect` i TUI, vælg `PhysicsCode Go`, og gå til [physicscode.ai/auth](https://physicscode.ai/zen).
...
Sådan bruger du Amazon Bedrock med PhysicsCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører physicscode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `physicscode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i PhysicsCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere physicscode til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at physicscode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem physicscode-konfigurationen.
...
gemme tokenet i physicscode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
PhysicsCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse PhysicsCode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`physicscode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [GitLab authentication helper](https://www.npmjs.com/package/opencode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `physicscode.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med PhysicsCode:
...
Sådan bruger du Google Vertex AI med PhysicsCode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører physicscode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem physicscode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [physicscode-helicone-session](https://github.com/H2Shami/physicscode-helicone-session) plugin for automatisk at logge hver PhysicsCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver PhysicsCode-samtale opført som en separat session.
...
Du kan konfigurere physicscode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere physicscode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere physicscode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for PhysicsCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/physicscode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med PhysicsCode:
...
3. Kopier API-nøglen til brug i PhysicsCode.
...
6. **Vigtigt**: Før du bruger skymodeller i PhysicsCode, skal du hente modelinformationen lokalt:
...
PhysicsCode Zen er en liste over testede og verificerede modeller leveret af PhysicsCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>PhysicsCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **PhysicsCode Zen**.

   ```txt
   /connect
   ```

3. Indtast PhysicsCode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem physicscode-konfigurationen.
...
5. Du kan også tilpasse dem gennem physicscode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med PhysicsCode:
...
Du kan også tilpasse modeller gennem physicscode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med physicscode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `physicscode.json`-filen i projektkataloget:
...
`limit`-felterne lader PhysicsCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `physicscode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek physicscode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i physicscode-konfigurationen.
