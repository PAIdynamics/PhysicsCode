---
title: Server
description: Komunicirajte s physicscode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

Komanda `physicscode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji physicscode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `physicscode.local` |
...
Postavite `PHYSICSCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `physicscode`, ili postavite `PHYSICSCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `physicscode serve` i za `physicscode web`.
...
Kada pokrenete `physicscode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite physicscode server za programsku interakciju sa physicscode-om.
:::

Ova arhitektura omogucava physicscode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `physicscode serve` da startate standalone server. Ako je physicscode TUI vec pokrenut, `physicscode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste PhysicsCode [IDE](/docs/ide) pluginovi.
...
physicscode server izlaže sljedece API-je.
