...
يوجد نوعان من الوكلاء في PhysicsCode: وكلاء أساسيون ووكلاء فرعيون.
...
يأتي PhysicsCode مع وكيلين أساسيين مدمجين: **Build** و **Plan**. سنلقي نظرة عليهما أدناه.
...
يأتي PhysicsCode مع وكيلين فرعيين مدمجين: **General** و **Explore**. سنلقي نظرة على ذلك أدناه.

---

## المدمجة

يأتي PhysicsCode مع وكيلين أساسيين مدمجين ووكيلين فرعيين مدمجين.
...
هيّئ الوكلاء في ملف إعدادات `physicscode.json`:
...
- عام: `~/.config/physicscode/agents/`
- لكل مشروع: `.physicscode/agents/`
...
إذا لم يتم تحديد temperature، يستخدم PhysicsCode القيم الافتراضية الخاصة بالنموذج؛ عادة 0 لمعظم النماذج و 0.55 لنماذج Qwen.
...
هذا المسار نسبي لمكان وجود ملف الإعدادات. لذلك يعمل هذا لكل من إعدادات PhysicsCode العامة وإعدادات المشروع.
...
يستخدم معرّف النموذج في إعدادات PhysicsCode الصيغة `provider/model-id`. على سبيل المثال، إذا كنت تستخدم [PhysicsCode Zen](/docs/zen)، فستستخدم `physicscode/gpt-5.1-codex` لـ GPT 5.1 Codex.
...
:::tip
شغّل `physicscode models` لعرض قائمة بالنماذج المتاحة.
:::
...
:::tip
هل لديك وكيل تود مشاركته؟ [قدّم PR](https://github.com/PAIdynamics/PhysicsCode).
:::
