...
Practical reference for extracting a future `packages/server` from the current `packages/physicscode` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all physicscode services, Effect-first source of truth
- `packages/server` - physicscode server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full physicscode server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/physicscode`.
...
- those slices are mounted into the Hono server behind `PHYSICSCODE_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `physicscode` package split is earlier in the migration.
...
- the current server host still lives in `packages/physicscode`
...
- if `packages/server` imports services from `packages/physicscode` while `packages/physicscode` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/physicscode` as the temporary Hono host
...
- `packages/server` must not import from `packages/physicscode`

Allowed in phase 1:

- `packages/physicscode` imports `packages/server`
...
- `packages/physicscode` shrinks or disappears as package responsibilities are fully split
...
- `packages/physicscode` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/physicscode`
...
Keep in `packages/physicscode` for now:
...
- host mounting from `packages/physicscode`
...
- no imports from `packages/physicscode` inside `packages/server`
...
- `packages/server` must still not import from `packages/physicscode`
...
### PR 4. Mount `packages/server` question from `packages/physicscode`

Scope:

- replace local experimental question route wiring in `packages/physicscode`
...
- mount it in parallel from `packages/physicscode`
...
- move adapter selection and server startup out of `packages/physicscode`

Rules:

- do not start this while `packages/server` still depends on `packages/physicscode`
...
- host logic no longer lives in `packages/physicscode`
...
3. the route is mounted from the current host in `packages/physicscode`
...
- `packages/server` imports services or runtime from `packages/physicscode`
- `packages/physicscode` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/physicscode` imports
...
- [ ] mount `question` from `packages/physicscode`
...
2. keep `packages/physicscode` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/physicscode`, stop and restructure the boundary first.
