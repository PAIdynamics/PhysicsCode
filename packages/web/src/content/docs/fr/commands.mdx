...
Créez `.physicscode/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration PhysicsCode ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre PhysicsCode [config](/docs/config) :
...
- Par projet : `.physicscode/commands/`
...
physicscode comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
