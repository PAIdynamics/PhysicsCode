...
const originalHttpApi = Flag.PHYSICSCODE_EXPERIMENTAL_HTTPAPI
const it = testEffect(
...
function request(path: string, directory: string, init: RequestInit = {}) {
  return Effect.promise(() => {
    ...
    headers.set("x-physicscode-directory", directory)
...
afterEach(async () => {
  ...
  Flag.PHYSICSCODE_EXPERIMENTAL_HTTPAPI = originalHttpApi
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-physicscode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://physicscode.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-physicscode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-physicscode-workspace")
      } finally {
