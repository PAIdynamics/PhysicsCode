...
description: Skriv dine egne programtillegg for å utvide PhysicsCode.
---

Plugins lar deg utvide PhysicsCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til PhysicsCode.
...
- `.physicscode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/physicscode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/physicscode/physicscode.json`)
2. Prosjektkonfigurasjon (`physicscode.json`)
3. Global plugin-katalog (`~/.config/physicscode/plugins/`)
4. Prosjektpluginkatalog (`.physicscode/plugins/`)
...
PhysicsCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En PhysicsCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide PhysicsCode.
...
:::note
Hvis du bruker PhysicsCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre PhysicsCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til PhysicsCode:
...
`tool`-hjelperen lager et tilpasset verktøy som PhysicsCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for PhysicsCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://physicscode.ai/docs/sdk) for detaljer.
