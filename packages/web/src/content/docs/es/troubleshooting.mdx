...
Para depurar problemas con PhysicsCode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/physicscode/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\physicscode\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `physicscode --log-level DEBUG`.

---

## Almacenamiento

physicscode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/physicscode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\physicscode`
...
PhysicsCode Desktop ejecuta un servidor PhysicsCode local (el sidecar `physicscode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `PhysicsCode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/physicscode/physicscode.jsonc` (o `~/.config/physicscode/physicscode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/physicscode/physicscode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\physicscode\physicscode.jsonc`
...
PhysicsCode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/physicscode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\physicscode\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.physicscode/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que PhysicsCode pueda reconstruirlo.

1. Salga de PhysicsCode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/physicscode`
- **Linux**: eliminar `~/.cache/physicscode` (o ejecutar `rm -rf ~/.cache/physicscode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\physicscode`

3. Reinicie el escritorio PhysicsCode.

---

### Solucionar problemas de conexión del servidor

PhysicsCode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `physicscode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `PHYSICSCODE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `PHYSICSCODE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio PhysicsCode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de PhysicsCode.

---

### Notificaciones que no se muestran

PhysicsCode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para PhysicsCode en la configuración de su sistema operativo, y
...
1. Salga del escritorio PhysicsCode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio PhysicsCode):

- `physicscode.settings.dat` (URL del servidor predeterminado de escritorio)
- `physicscode.global.dat` y `physicscode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con PhysicsCode:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/PAIdynamics/PhysicsCode/issues**](https://github.com/PAIdynamics/PhysicsCode/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**physicscode.ai/discord**](https://physicscode.ai/discord)
...
3. Asegúrese de tener la última versión con `physicscode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `physicscode models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/physicscode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\physicscode`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. physicscode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   ```bash
   rm -rf ~/.cache/physicscode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\physicscode`

2. Reinicie physicscode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a physicscode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
physicscode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
