...
You can define MCP servers in your [PhysicsCode Config](https://physicscode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
`physicscode account login` and reuse that session's token):
