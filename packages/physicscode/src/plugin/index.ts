...
export class Service extends Context.Service<Service, Interface>()("@physicscode/Plugin") {}
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        const client = createOpencodeClient({
          ...
          headers: Flag.PHYSICSCODE_SERVER_PASSWORD
            ? {
                Authorization: `Basic ${Buffer.from(`${Flag.PHYSICSCODE_SERVER_USERNAME ?? "physicscode"}:${Flag.PHYSICSCODE_SERVER_PASSWORD}`).toString("base64")}`,
              }
            : undefined,
        ...
        const plugins = Flag.PHYSICSCODE_PURE ? [] : (cfg.plugin_origins ?? [])
        if (Flag.PHYSICSCODE_PURE && cfg.plugin_origins?.length) {
