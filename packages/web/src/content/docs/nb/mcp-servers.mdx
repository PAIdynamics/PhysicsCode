...
Du kan legge til eksterne verktøy i PhysicsCode ved å bruke _Model Context Protocol_ eller MCP. PhysicsCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [PhysicsCode Config](https://physicscode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/physicscode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
PhysicsCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil PhysicsCode:
...
Hvis serveren krever autentisering, vil PhysicsCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `physicscode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil PhysicsCode lagre tokens sikkert i `~/.local/share/physicscode/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i PhysicsCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom PhysicsCode-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble PhysicsCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver PhysicsCode-samtale oppført som en separat økt.
