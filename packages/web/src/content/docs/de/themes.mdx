...
Mit PhysicsCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet PhysicsCode das `physicscode`-Theme.
...
PhysicsCode liefert mehrere eingebaute Themes mit.
...
- PhysicsCode optisch ans Terminal anpassen wollen
...
PhysicsCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/physicscode/themes/*.json` oder `$XDG_CONFIG_HOME/physicscode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.physicscode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.physicscode/themes/*.json`
