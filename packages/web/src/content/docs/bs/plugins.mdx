---
title: Dodaci
description: Napišite vlastite dodatke za proširenje PhysicsCode.
---

Dodaci vam omogućavaju da proširite PhysicsCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja PhysicsCode.
...
- `.physicscode/plugins/` - Dodaci na nivou projekta
- `~/.config/physicscode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/physicscode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/physicscode/physicscode.json`)
2. Konfiguracija projekta (`physicscode.json`)
3. Globalni direktorij dodataka (`~/.config/physicscode/plugins/`)
4. Direktorij dodataka projekta (`.physicscode/plugins/`)
...
PhysicsCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje PhysicsCode.
...
Spriječite physicscode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u physicscode:
...
Pomoćnik `tool` kreira prilagođeni alat koji physicscode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za physicscode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://physicscode.ai/docs/sdk) za detalje.
