...
PhysicsCode Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

:::note
PhysicsCode Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Provider in PhysicsCode. Du abonnierst PhysicsCode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
PhysicsCode zu verwenden.
...
PhysicsCode Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

PhysicsCode Go funktioniert wie jeder andere Provider in PhysicsCode.

1. Du meldest dich bei **<a href={console}>PhysicsCode Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `PhysicsCode Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann PhysicsCode Go abonnieren.
:::
...
PhysicsCode Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner PhysicsCode Config verwendet das Format `physicscode-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `physicscode-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben PhysicsCode Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit PhysicsCode zu nutzen.
