---
title: Server
description: Interact with physicscode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

The `physicscode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an physicscode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `physicscode.local` |
...
Set `PHYSICSCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `physicscode`, or set `PHYSICSCODE_SERVER_USERNAME` to override it. This applies to both `physicscode serve` and `physicscode web`.
...
When you run `physicscode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the physicscode server to interact with physicscode programmatically.
:::

This architecture lets physicscode support multiple clients and allows you to interact with physicscode programmatically.

You can run `physicscode serve` to start a standalone server. If you have the
physicscode TUI running, `physicscode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the PhysicsCode [IDE](/docs/ide) plugins.
...
The physicscode server exposes the following APIs.
