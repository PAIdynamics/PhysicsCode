...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("physicscode-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("physicscode_lib=debug,physicscode_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("physicscode_lib=info,physicscode_desktop=info,sidecar=info")
        }
    });
