...
export const DEPRECATED_PLUGIN_PACKAGES = ["physicscode-openai-codex-auth", "physicscode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, physicscodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(physicscodeVersion) || semver.major(physicscodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.physicscode
  if (typeof range !== "string") return
  if (!semver.satisfies(physicscodeVersion, range)) {
    throw new Error(`Plugin requires physicscode ${range} but running ${physicscodeVersion}`)
  }
}
