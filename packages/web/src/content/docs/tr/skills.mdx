...
Ajan becerileri, physicscode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
physicscode su konumlari tarar:

- Proje konfigurasyonu: `.physicscode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/physicscode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda physicscode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.physicscode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/physicscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.physicscode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
physicscode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`physicscode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`physicscode.json` icinde):
