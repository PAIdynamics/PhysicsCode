...
U PhysicsCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, PhysicsCode koristi nasu `physicscode` temu.
...
PhysicsCode dolazi sa vise ugradenih tema.
...
- Zele da PhysicsCode odgovara izgledu njihovog terminala
...
PhysicsCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/physicscode/themes/*.json` ili `$XDG_CONFIG_HOME/physicscode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.physicscode/themes/*.json`
4. **Trenutni radni direktorij** - `./.physicscode/themes/*.json`
