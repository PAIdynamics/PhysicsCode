...
Umiejętności agenta pozwalają physicscode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
physicscode przeszukuje te lokalizacje:

- Project config: `.physicscode/skills/<name>/SKILL.md`
- Global config: `~/.config/physicscode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu physicscode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.physicscode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/physicscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.physicscode/skills/git-release/SKILL.md` w ten sposób:
...
physicscode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `physicscode.json`:
...
**Dla agentów wbudowanych** (w `physicscode.json`):
