---
title: Serwer
description: Interakcja z serwerem physicscode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

Komenda `physicscode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient physicscode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `physicscode.local` |
...
Ustaw `PHYSICSCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `physicscode` lub ustaw `PHYSICSCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `physicscode serve`, jak i `physicscode web`.
...
Kiedy uruchomisz `physicscode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera physicscode do programowej interakcji z physicscode.
:::

Ta architektura umożliwia obsługę wielu klientów przez physicscode i programową interakcję z kodem otwartym.

Możesz uruchomić `physicscode serve`, aby uruchomić samodzielny serwer. Jeśli masz
physicscode TUI działa, `physicscode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki physicscode [IDE](/docs/ide).
...
Serwer physicscode udostępnia następujące interfejsy API.
