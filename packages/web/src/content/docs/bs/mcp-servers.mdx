...
Možete dodati vanjske alate u PhysicsCode koristeći _Model Context Protocol_, ili MCP. PhysicsCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [PhysicsCode Config](https://physicscode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/physicscode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
PhysicsCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, PhysicsCode će:
...
Ako server zahtijeva autentifikaciju, PhysicsCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `physicscode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, PhysicsCode sigurno čuva tokene u `~/.local/share/physicscode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u PhysicsCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz PhysicsCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete PhysicsCode sa vašim Sentry nalogom.
