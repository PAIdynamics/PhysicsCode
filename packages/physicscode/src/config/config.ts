...
import { InstallationLocal, InstallationVersion } from "@physicscode-ai/core/installation/version"
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for physicscode serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://physicscode.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://physicscode.ai/docs/agents" }),
...
export class Service extends Context.Service<Service, Interface>()("@physicscode/Config") {}

function globalConfigFile() {
  const candidates = ["physicscode.jsonc", "physicscode.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://physicscode.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "physicscode.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "physicscode.jsonc")))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://physicscode.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "PHYSICSCODE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/physicscode` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/physicscode`))
            if (!response.ok) {
              throw new Error(`failed to fetch remote config from ${url}: ${response.status}`)
            }
            const wellknown = (yield* Effect.promise(() => response.json())) as { config?: Record<string, unknown> }
            const remoteConfig = wellknown.config ?? {}
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://physicscode.ai/config.json"
            const source = `${url}/.well-known/physicscode`
        ...
        if (Flag.PHYSICSCODE_CONFIG) {
          yield* merge(Flag.PHYSICSCODE_CONFIG, yield* loadFile(Flag.PHYSICSCODE_CONFIG))
          log.debug("loaded custom config", { path: Flag.PHYSICSCODE_CONFIG })
        }

        if (!Flag.PHYSICSCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("physicscode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.PHYSICSCODE_CONFIG_DIR) {
          log.debug("loading config from PHYSICSCODE_CONFIG_DIR", { path: Flag.PHYSICSCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []
        const bootstrapPluginSdk = !InstallationLocal && !InstallationVersion.startsWith("0.0.0-")

        for (const dir of directories) {
          if (dir.endsWith(".physicscode") || dir === Flag.PHYSICSCODE_CONFIG_DIR) {
            for (const file of ["physicscode.json", "physicscode.jsonc"]) {
          ...
          if (bootstrapPluginSdk) {
            const dep = yield* npmSvc
              .install(dir, {
                add: [
                  {
                    name: "@physicscode-ai/plugin",
          ...
          // Auto-discovered plugins under `.physicscode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.PHYSICSCODE_CONFIG_CONTENT) {
          const source = "PHYSICSCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.PHYSICSCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["PHYSICSCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("PHYSICSCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.PHYSICSCODE_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.PHYSICSCODE_PERMISSION))
        }
        ...
        if (Flag.PHYSICSCODE_DISABLE_AUTOCOMPACT) {
