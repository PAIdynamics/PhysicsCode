...
Le skill degli agenti permettono a PhysicsCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
PhysicsCode cerca in queste posizioni:

- Config di progetto: `.physicscode/skills/<name>/SKILL.md`
- Config globale: `~/.config/physicscode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, PhysicsCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.physicscode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/physicscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.physicscode/skills/git-release/SKILL.md` cosi':
...
PhysicsCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `physicscode.json`:
...
**Per agenti integrati** (in `physicscode.json`):
