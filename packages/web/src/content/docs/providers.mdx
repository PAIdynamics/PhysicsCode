...
PhysicsCode Zen is a list of models provided by the PhysicsCode team that have been
tested and verified to work well with PhysicsCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with PhysicsCode Zen.
:::

1. Run the `/connect` command in the TUI, select `PhysicsCode Zen`, and head to [physicscode.ai/auth](https://physicscode.ai/zen).
...
PhysicsCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the PhysicsCode team that have been
tested and verified to work well with PhysicsCode.

1. Run the `/connect` command in the TUI, select `PhysicsCode Go`, and head to [physicscode.ai/auth](https://physicscode.ai/zen).
...
## PhysicsCode Credit (OpenAI / Anthropic)

If you're logged in to a PhysicsCode account, you can run real OpenAI and
Anthropic models through PhysicsCode's proxy without bringing your own vendor
API key — usage is billed against your PhysicsCode account credit at the
vendor's real pricing instead.
...
```json title="physicscode.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://physicscode.ai/config.json",
  "provider": {
    "atomic-chat": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [physicscode-helicone-session](https://github.com/H2Shami/physicscode-helicone-session) plugin to automatically log each PhysicsCode conversation as a session in Helicone.

```bash
npm install -g physicscode-helicone-session
```
...
```json title="physicscode.json"
{
  "plugin": ["physicscode-helicone-session"]
}
```
...
```json title="physicscode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://physicscode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
```json title="physicscode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://physicscode.ai/config.json",
  "provider": {
    "lmstudio": {
...
:::tip
Ollama can automatically configure itself for PhysicsCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/physicscode) for details.
:::

```json title="physicscode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://physicscode.ai/config.json",
  "provider": {
    "ollama": {
...
```json title="physicscode.json" {9,11,17-20}
{
  "$schema": "https://physicscode.ai/config.json",
  "provider": {
    "myprovider": {
