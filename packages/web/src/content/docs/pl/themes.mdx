...
Dzięki physicscode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie physicscode używa naszego własnego motywu `physicscode`.
...
physicscode ma kilka wbudowanych motywów.
...
- Chcą, aby PhysicsCode pasował do wyglądu ich terminala
...
physicscode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/physicscode/themes/*.json` lub `$XDG_CONFIG_HOME/physicscode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.physicscode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.physicscode/themes/*.json`
