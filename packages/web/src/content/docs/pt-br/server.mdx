---
title: Servidor
description: Interaja com o servidor physicscode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

O comando `physicscode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente physicscode pode usar.
...
```bash
physicscode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `physicscode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
physicscode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `PHYSICSCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `physicscode`, ou defina `PHYSICSCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `physicscode serve` quanto ao `physicscode web`.

```bash
PHYSICSCODE_SERVER_PASSWORD=your-password physicscode serve
```
...
Quando você executa `physicscode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor physicscode para interagir com o physicscode programaticamente.
:::

Essa arquitetura permite que o physicscode suporte múltiplos clientes e permite que você interaja com o physicscode programaticamente.

Você pode executar `physicscode serve` para iniciar um servidor autônomo. Se você tiver o TUI do physicscode em execução, `physicscode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do physicscode [IDE](/docs/ide).
...
O servidor physicscode expõe as seguintes APIs.
