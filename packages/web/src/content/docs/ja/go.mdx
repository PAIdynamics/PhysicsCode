...
PhysicsCode Goは低価格のサブスクリプションで、**初月は5ドル**、その後は**月額10ドル**で、人気のオープンなコーディングモデルに安定してアクセスできます。

:::note
PhysicsCode Goは現在ベータ版です。
:::

GoはPhysicsCodeの他のプロバイダーと同様に機能します。PhysicsCode GoをサブスクライブしてAPIキーを取得します。これは**完全に任意**であり、PhysicsCodeを使用するために必須ではありません。
...
:::tip
PhysicsCodeでうまく動作する一部のモデルとプロバイダーをテストしました。
:::
...
PhysicsCode Goを使用すると、これらのモデルに**初月は5ドル**、その後は**月額10ドル**でアクセスできます。

---

## 仕組み

PhysicsCode Goは、PhysicsCodeの他のプロバイダーと同様に機能します。

1. **<a href={console}>PhysicsCode Zen</a>**にサインインし、GoをサブスクライブしてAPIキーをコピーします。
2. TUIで`/connect`コマンドを実行し、`PhysicsCode Go`を選択して、APIキーを貼り付けます。
3. TUIで`/models`を実行すると、Goを通じて利用可能なモデルのリストが表示されます。

:::note
PhysicsCode Goをサブスクライブできるのは、1つのワークスペースにつき1メンバーのみです。
:::
...
PhysicsCode設定の[model id](/docs/config/#models)は、`physicscode-go/<model-id>`という形式を使用します。たとえば、Kimi K2.6の場合は、設定で`physicscode-go/kimi-k2.6`を使用します。
...
4. PhysicsCodeで他のプロバイダーも使用できるようにすることで、**ベンダーロックインをなくす**。
