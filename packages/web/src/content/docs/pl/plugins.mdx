---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć physicscode.
---

Wtyczki umożliwiające wyłączenie physicscode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania physicscode.
...
- `.physicscode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/physicscode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/physicscode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/physicscode/physicscode.json`)
2. Konfiguracja projektu (`physicscode.json`)
3. Globalny katalog wtyczek (`~/.config/physicscode/plugins/`)
4. Katalog wtyczek projektu (`.physicscode/plugins/`)
...
physicscode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z physicscode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do physicscode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej physicscode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj physicscode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do physicscode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod physicscode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://physicscode.ai/docs/sdk).
