...
Agent Skills erlauben PhysicsCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
PhysicsCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft PhysicsCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.physicscode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/physicscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.physicscode/skills/git-release/SKILL.md` zum Beispiel so:
...
PhysicsCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `physicscode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `physicscode.json`):
