---
title: Servidor
description: Interactuar con el servidor physicscode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

El comando `physicscode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente physicscode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `physicscode.local` |
...
Configure `PHYSICSCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `physicscode`, o configure `PHYSICSCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `physicscode serve` como a `physicscode web`.
...
Cuando ejecuta `physicscode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor physicscode para interactuar con physicscode mediante programación.
:::

Esta arquitectura permite que physicscode admita múltiples clientes y le permite interactuar con physicscode mediante programación.

Puede ejecutar `physicscode serve` para iniciar un servidor independiente. Si tienes el
physicscode TUI ejecutándose, `physicscode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos PhysicsCode [IDE](/docs/ide).
...
El servidor physicscode expone las siguientes API.
