...
In PhysicsCode esistono due tipi di agenti: agenti primari e subagenti.
...
PhysicsCode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
PhysicsCode include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

PhysicsCode include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `physicscode.json`:
...
- Globale: `~/.config/physicscode/agents/`
- Per progetto: `.physicscode/agents/`
...
Se non specifichi una temperature, PhysicsCode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di PhysicsCode sia per la config specifica del progetto.
...
L'ID modello nella configurazione PhysicsCode usa il formato `provider/model-id`. Per esempio, se stai usando [PhysicsCode Zen](/docs/zen), useresti `physicscode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `physicscode models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/PAIdynamics/PhysicsCode).
:::
