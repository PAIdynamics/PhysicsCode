...
Wenn PhysicsCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\physicscode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `physicscode --log-level DEBUG`.

---

## Speicher

physicscode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/physicscode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\physicscode`
...
PhysicsCode Desktop startet im Hintergrund einen lokalen PhysicsCode-Server (`physicscode-cli`-Sidecar).
...
- Nur macOS: `PhysicsCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/physicscode/physicscode.jsonc` (oder `~/.config/physicscode/physicscode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/physicscode/physicscode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\physicscode\physicscode.jsonc`
...
PhysicsCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/physicscode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\physicscode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit PhysicsCode ihn neu aufbauen kann.

1. Beende PhysicsCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/physicscode`
- **Linux**: loesche `~/.cache/physicscode` (oder fuehre aus: `rm -rf ~/.cache/physicscode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\physicscode`

3. Starte PhysicsCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

PhysicsCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `physicscode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `PHYSICSCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `PHYSICSCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt PhysicsCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit PhysicsCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

PhysicsCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer PhysicsCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende PhysicsCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von PhysicsCode Desktop):

- `physicscode.settings.dat` (desktop default server URL)
- `physicscode.global.dat` und `physicscode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit PhysicsCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `physicscode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\physicscode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. physicscode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\physicscode`

2. Starte physicscode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt physicscode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
physicscode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
