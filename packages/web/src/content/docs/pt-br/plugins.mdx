---
title: Plugins
description: Escreva seus próprios plugins para estender o physicscode.
---

Plugins permitem que você estenda o physicscode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do physicscode.
...
- `.physicscode/plugins/` - Plugins em nível de projeto
- `~/.config/physicscode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/physicscode/node_modules/`.
...
1. Configuração global (`~/.config/physicscode/physicscode.json`)
2. Configuração do projeto (`physicscode.json`)
3. Diretório de plugins global (`~/.config/physicscode/plugins/`)
4. Diretório de plugins do projeto (`.physicscode/plugins/`)
...
O physicscode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do physicscode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o physicscode.
...
:::note
Se você estiver usando o aplicativo desktop physicscode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o physicscode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao physicscode:
...
O helper `tool` cria uma ferramenta personalizada que o physicscode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o physicscode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://physicscode.ai/docs/sdk) para detalhes.
