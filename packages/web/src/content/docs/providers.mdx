---
title: Providers
description: Using any LLM provider in PhysicsCode.
---

import config from "../../../config.mjs"
export const console = config.console

PhysicsCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your PhysicsCode config.
...
in `~/.local/share/physicscode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your PhysicsCode
config.
...
## PAI Dynamics Hosted (Zen)

PhysicsCode's default, no-setup model provider — log in and go, no separate
API key or billing setup needed. [Learn more](/docs/zen).
...
1. Run `physicscode account login` (or `/connect` in the TUI and select **PAI
   Dynamics Hosted**).
2. Run `/models` to see what's currently being served.

It works like any other provider in PhysicsCode and is completely optional to use.

---

## PhysicsCode Credit for OpenAI/Anthropic (Go)

Run real OpenAI and Anthropic models through PhysicsCode's proxy without
bringing your own vendor API key — billed against your PhysicsCode account
credit at the vendor's real pricing instead. [Learn more](/docs/go).

1. Log in with `physicscode account login` (or `/connect` in the TUI).
2. Select `PhysicsCode Credit (OpenAI)` or `PhysicsCode Credit (Anthropic)` —
   no separate API key to paste, it uses your logged-in account.
3. Run `/models` to see what's available
   (`physicscode-openai/gpt-5.6`, `physicscode-anthropic/claude-opus-4-8`,
   etc).

Requests stop once your credit balance reaches zero, and a single request
estimated to cost more than the configured per-request limit is rejected
before it's sent rather than after. Check your balance and usage history in
the account portal's Billing tab.

This depends on the server operator having configured real vendor API keys;
until then, selecting one of these providers returns a
...
To use Amazon Bedrock with PhysicsCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running physicscode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY physicscode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile physicscode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX physicscode
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `physicscode.json`:

   ```json title="physicscode.json"
   {
     "provider": {
       "amazon-bedrock": {
         "options": {
...
PhysicsCode. Anthropic explicitly prohibits this.

Previous versions of PhysicsCode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in PhysicsCode with zero setup:
...
You can configure physicscode to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="physicscode.json" "atomic-chat" {4,5,7,9-13}
{
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for physicscode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX physicscode
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX physicscode
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your physicscode config.
...
GitLab Duo support in PhysicsCode is experimental. Features, configuration, and
behavior may change in future releases.
:::

PhysicsCode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in physicscode auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
PhysicsCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock PhysicsCode
to only use your own GitLab-hosted instance, add the following to your
`physicscode.json` file. It is also recommended to disable session sharing.
...
More documentation on [GitLab authentication helper](https://www.npmjs.com/package/opencode-gitlab-auth) homepage.

##### Configuration

Customize through `physicscode.json`:
...
When a `duo-workflow-*` model is selected, PhysicsCode will:
...
4. Route tool execution requests through PhysicsCode's permission-gated tool system
...
```json title="physicscode.json"
{
  "plugin": ["physicscode-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with physicscode:
...
To use Google Vertex AI with PhysicsCode:
...
2. Set the required environment variables:
   ...
   Set them while running physicscode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id physicscode
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through physicscode, you can always configure it yourself.
...
```jsonc title="~/.config/physicscode/physicscode.jsonc"
{
  "provider": {
    "helicone": {
      "npm": "@ai-sdk/openai-compatible",
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each PhysicsCode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each PhysicsCode conversation listed as a separate session.
...
You can configure physicscode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="physicscode.json" "llama.cpp" {4,5,7,9-14}
{
  "provider": {
    "llama.cpp": {
      "npm": "@ai-sdk/openai-compatible",
...
You can configure physicscode to use local models through LM Studio.

```json title="physicscode.json" "lmstudio" {4,5,7,9-13}
{
  "provider": {
    "lmstudio": {
      "npm": "@ai-sdk/openai-compatible",
...
You can configure physicscode to use local models through Ollama.

:::tip
Ollama has an auto-configuration integration for opencode (the upstream
project PhysicsCode is forked from) — see the
[Ollama integration docs](https://docs.ollama.com/integrations/opencode) for
details. It hasn't been verified against this fork specifically, so the
manual config below is the reliable path.
:::

```json title="physicscode.json" "ollama" {4,5,7,9-13}
{
  "provider": {
    "ollama": {
      "npm": "@ai-sdk/openai-compatible",
...
To reach Ollama running on another machine (or a container) instead of locally, start it with `OLLAMA_HOST=0.0.0.0 ollama serve` on that machine, then point `options.baseURL` at its address, e.g. `http://192.168.1.50:11434/v1`.

---

### Ollama Cloud

To use Ollama Cloud with PhysicsCode:
...
3. Copy the API key for use in PhysicsCode.
...
6. **Important**: Before using cloud models in PhysicsCode, you must pull the model information locally:
...
PhysicsCode Zen is a list of tested and verified models provided by the PhysicsCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>PhysicsCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **PhysicsCode Zen**.

   ```txt
   /connect
   ```

3. Enter your PhysicsCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your physicscode config.

   ```json title="physicscode.json" {6}
   {
     "provider": {
       "openrouter": {
         "models": {
...
5. You can also customize them through your physicscode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' physicscode
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group physicscode
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with PhysicsCode:
...
You can also customize models through your physicscode config. Here's an example of specifying provider routing order.
...
[vLLM](https://docs.vllm.ai) serves an OpenAI-compatible API, so it works the same way whether it's running on the machine you're using physicscode from or on another machine on your network (e.g. a GPU server).

1. Start vLLM. By default it only binds to `localhost`; pass `--host 0.0.0.0` to make it reachable from other machines.

   ```bash
   vllm serve Qwen/Qwen2.5-Coder-32B-Instruct --host 0.0.0.0 --port 8000
   ```

2. Point physicscode at it. Use `127.0.0.1` if vLLM is on the same machine, or that machine's address otherwise.

   ```json title="physicscode.json" "vllm" {4,5,7,9-13}
   {
     "provider": {
       "vllm": {
         "npm": "@ai-sdk/openai-compatible",
   ...
   - `models` keys must match the model ID vLLM was started with — run `curl http://192.168.1.50:8000/v1/models` to confirm.

3. If vLLM (or a proxy in front of it) requires an API key, add it to `options`:

   ```json title="physicscode.json" {4}
   {
     "options": {
       "baseURL": "http://192.168.1.50:8000/v1",
...
:::tip
You can use any OpenAI-compatible provider with physicscode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in physicscode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `physicscode.json` file in your project directory:

   ```json title="physicscode.json" ""myprovider"" {5-15}
   {
     "provider": {
       "myprovider": {
         "npm": "@ai-sdk/openai-compatible",
...
```json title="physicscode.json" {8,10,16-19}
{
  "provider": {
    "myprovider": {
      "npm": "@ai-sdk/openai-compatible",
...
The `limit` fields allow PhysicsCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `physicscode auth list` to see if the credentials
...
2. For custom providers, check the physicscode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your physicscode config.
