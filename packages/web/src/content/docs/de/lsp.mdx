---
title: LSP Server
description: PhysicsCode lässt sich in Ihren LSP-Server integrieren.
---

PhysicsCode lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

PhysicsCode verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `PHYSICSCODE_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn PhysicsCode eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer PhysicsCode-Konfiguration anpassen.
...
```json title="physicscode.json" {5-7}
{
  "lsp": {
    "rust": {
      "env": {
...
```json title="physicscode.json" {5-9}
{
  "lsp": {
    "typescript": {
      "initialization": {
