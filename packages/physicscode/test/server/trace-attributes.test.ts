...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under physicscode.", () => {
    expect(paramToAttributeKey("name")).toBe("physicscode.name")
    expect(paramToAttributeKey("slug")).toBe("physicscode.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["physicscode.sessionID"]).toBeUndefined()
    expect(attrs["physicscode.messageID"]).toBeUndefined()
    expect(attrs["physicscode.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under physicscode. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["physicscode.name"]).toBe("exa")
