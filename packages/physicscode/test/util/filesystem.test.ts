...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "physicscode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "physicscode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "physicscode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "physicscode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["physicscode.json", "physicscode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "physicscode.json"),
        path.join(tmp.path, "physicscode.jsonc"),
        path.join(project, "physicscode.json"),
        path.join(project, "physicscode.jsonc"),
      ])
    })
  })
