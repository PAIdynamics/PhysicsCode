---
title: サーバー
description: HTTP 経由で PhysicsCode サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

`physicscode serve` コマンドは、PhysicsCode クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `physicscode.local` |
...
HTTP 基本認証でサーバーを保護するには、`PHYSICSCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `physicscode` になるか、`PHYSICSCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`physicscode serve` と `physicscode web` の両方に当てはまります。
...
`physicscode` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
PhysicsCode サーバーを使用して、プログラムで PhysicsCode と対話します。
:::

`physicscode serve` を実行してスタンドアロンサーバーを起動できます。すでに PhysicsCode TUI を実行している場合でも、`physicscode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、PhysicsCode [IDE](/docs/ide) プラグインによって使用されます。
...
PhysicsCode サーバーは次の API を公開します。
