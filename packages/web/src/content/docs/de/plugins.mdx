---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um PhysicsCode zu erweitern.
---

Mit Plugins können Sie PhysicsCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von PhysicsCode zu ändern.
...
- `.physicscode/plugins/` – Plugins auf Projektebene
- `~/.config/physicscode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/physicscode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/physicscode/physicscode.json`)
2. Projektkonfiguration (`physicscode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/physicscode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.physicscode/plugins/`)
...
PhysicsCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein PhysicsCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie PhysicsCode erweitern können.
...
:::note
Wenn Sie die PhysicsCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass PhysicsCode `.env`-Dateien liest:
...
Plugins können PhysicsCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das PhysicsCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für PhysicsCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://physicscode.ai/docs/sdk).
