...
description: Cree herramientas que LLM pueda llamar en physicscode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de physicscode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.physicscode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/physicscode/tools/`.
...
```ts title=".physicscode/tools/database.ts" {1}
...
```ts title=".physicscode/tools/math.ts"
import { tool } from "@physicscode-ai/plugin"
...
```ts title=".physicscode/tools/bash.ts"
import { tool } from "@physicscode-ai/plugin"
...
```ts title=".physicscode/tools/project.ts" {8}
import { tool } from "@physicscode-ai/plugin"
...
```python title=".physicscode/tools/add.py"
import sys
...
```ts title=".physicscode/tools/python-add.ts" {10}
import { tool } from "@physicscode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".physicscode/tools/add.py")
