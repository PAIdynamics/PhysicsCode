...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. PhysicsCode include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in PhysicsCode.
...
:::note
Questo strumento e' disponibile solo quando `PHYSICSCODE_EXPERIMENTAL_LSP_TOOL=true` (oppure `PHYSICSCODE_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider PhysicsCode o quando la variabile d'ambiente `PHYSICSCODE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di PhysicsCode:
