---
title: Server
description: Interagisci con il server physicscode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

Il comando `physicscode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client physicscode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `physicscode.local` |
...
Imposta `PHYSICSCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `physicscode`, oppure imposta `PHYSICSCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `physicscode serve` sia per `physicscode web`.
...
Quando esegui `physicscode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server physicscode per interagire con physicscode in modo programmatico.
:::

Questa architettura permette a physicscode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `physicscode serve` per avviare un server standalone. Se la TUI di physicscode e gia in esecuzione, `physicscode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di PhysicsCode.
...
Il server physicscode espone le seguenti API.
