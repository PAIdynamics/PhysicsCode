---
title: Konfiguration
description: Ved at bruge PhysicsCode JSON konfig.
---

Du kan konfigurere PhysicsCode ved hjælp af en JSON-konfigurationsfil.

---

## Format

PhysicsCode understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/physicscode`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/physicscode/physicscode.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`PHYSICSCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`physicscode.json` i projekt) - projektspecifikke indstillinger
5. **`.physicscode` mapper** - agent, kommandoer, plugins
6. **Inline config** (`PHYSICSCODE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.physicscode` og `~/.config/physicscode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/physicscode`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale PhysicsCode-konfiguration i `~/.config/physicscode/physicscode.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `physicscode.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når PhysicsCode starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `PHYSICSCODE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `PHYSICSCODE_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.physicscode` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.physicscode` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`physicscode.ai/config.json`**](https://physicscode.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `physicscode serve` og `physicscode web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din PhysicsCode-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `physicscode.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din PhysicsCode-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger PhysicsCode at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din PhysicsCode-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/physicscode/agents/` eller `.physicscode/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil PhysicsCode falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`physicscode run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/physicscode/commands/` eller `.physicscode/commands/`. [Learn more here](/docs/commands).
...
PhysicsCode vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard physicscode **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide PhysicsCode med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.physicscode/plugins/` eller `~/.config/physicscode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse PhysicsCode til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
