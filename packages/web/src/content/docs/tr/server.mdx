---
title: Sunucu
description: physicscode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/main/packages/sdk/js/src/gen/types.gen.ts`

`physicscode serve` komutu, physicscode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `physicscode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `PHYSICSCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `physicscode` değeridir; değiştirmek isterseniz `PHYSICSCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `physicscode serve` hem de `physicscode web` için geçerlidir.
...
`physicscode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
physicscode ile programatik etkileşim için physicscode sunucusunu kullanın.
:::

Bu mimari, physicscode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`physicscode serve` ile bağımsız bir sunucu başlatabilirsiniz. physicscode TUI açıksa, `physicscode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum physicscode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
physicscode sunucusu aşağıdaki API'leri sunar.
