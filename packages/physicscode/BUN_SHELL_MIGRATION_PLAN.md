...
Replace runtime Bun shell template-tag usage in `packages/physicscode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/physicscode/src` is removed except:
