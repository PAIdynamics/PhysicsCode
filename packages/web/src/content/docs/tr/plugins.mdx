---
title: Eklentiler
description: PhysicsCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek PhysicsCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya PhysicsCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.physicscode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/physicscode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/physicscode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/physicscode/physicscode.json`)
2. Project config (`physicscode.json`)
3. Global eklenti dizini (`~/.config/physicscode/plugins/`)
4. Proje eklenti dizini (`.physicscode/plugins/`)
...
PhysicsCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir PhysicsCode SDK istemcisi.
...
PhysicsCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
PhysicsCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
PhysicsCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca PhysicsCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, PhysicsCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra PhysicsCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://physicscode.ai/docs/sdk) bakın.
