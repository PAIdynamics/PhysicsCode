---
title: TUI
description: PhysicsCode terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

PhysicsCode, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

PhysicsCode'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
PhysicsCode TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
PhysicsCode'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
PhysicsCode'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `physicscode.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `PHYSICSCODE_TUI_CONFIG` kullanın.
