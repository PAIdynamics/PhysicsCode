...
const originalWorkspacesFlag = Flag.PHYSICSCODE_EXPERIMENTAL_WORKSPACES
const originalEnv = {
  PHYSICSCODE_AUTH_CONTENT: process.env.PHYSICSCODE_AUTH_CONTENT,
...
beforeEach(() => {
  Database.close()
  Flag.PHYSICSCODE_EXPERIMENTAL_WORKSPACES = true
  restoreEnv()
})
...
afterEach(async () => {
  ...
  Flag.PHYSICSCODE_EXPERIMENTAL_WORKSPACES = originalWorkspacesFlag
  restoreEnv()
  await resetDatabase()
})
...
describe("workspace-old CRUD", () => {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async (dir) => {
      process.env.PHYSICSCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
      process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
      process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
      process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=physicscode-test"
      ...
      expect(JSON.parse(recorded.calls.create[0].env.PHYSICSCODE_AUTH_CONTENT ?? "{}")).toEqual({
        test: { type: "api", key: "secret" },
      })
      expect(recorded.calls.create[0].env.PHYSICSCODE_WORKSPACE_ID).toBe(workspaceID)
      expect(recorded.calls.create[0].env.PHYSICSCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
      ...
      expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=physicscode-test")
...
describe("workspace-old sync state", () => {
  test("startWorkspaceSyncing is disabled by the experimental workspace flag", async () => {
    await withInstance(async (dir) => {
      Flag.PHYSICSCODE_EXPERIMENTAL_WORKSPACES = false
