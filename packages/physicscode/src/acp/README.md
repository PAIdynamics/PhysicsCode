...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for physicscode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal physicscode sessions
...
physicscode acp --cwd /path/to/project
```
...
```bash
PHYSICSCODE_ENABLE_QUESTION_TOOL=1 physicscode acp
```
...
- **Terminal Integration**: Full terminal support via physicscode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | physicscode acp
```
...
### Mapping to PhysicsCode

ACP sessions map cleanly to physicscode's internal session model:
