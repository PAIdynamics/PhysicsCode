...
description: O physicscode integra-se aos seus servidores LSP.
---

O physicscode integra-se ao seu Protocolo de Servidor de Linguagem (LSP) para ajudar o LLM a interagir com sua base de código. Ele usa diagnósticos para fornecer feedback ao LLM.

---

## Integrados

O physicscode vem com vários servidores LSP integrados para linguagens populares:
...
:::note
Você pode desabilitar os downloads automáticos do servidor LSP definindo a variável de ambiente `PHYSICSCODE_DISABLE_LSP_DOWNLOAD` como `true`.
:::
...
Quando o physicscode abre um arquivo, ele:
...
Você pode personalizar os servidores LSP através da seção `lsp` na sua configuração do physicscode.
