...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall physicscode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall PhysicsCode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g physicscode-ai",
      pnpm: "pnpm uninstall -g physicscode-ai",
      bun: "bun remove -g physicscode-ai",
      yarn: "yarn global remove physicscode-ai",
      brew: "brew uninstall physicscode",
      choco: "choco uninstall physicscode",
      scoop: "scoop uninstall physicscode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "physicscode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".physicscode")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using PhysicsCode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# physicscode") || content.includes(".physicscode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".physicscode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".physicscode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".physicscode"))
    ) {
      continue
    }
