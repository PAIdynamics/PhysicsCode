...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const physicscodeModel = {
      ...openaiModel,
      providerID: "physicscode",
      api: {
        id: "physicscode-test",
        url: "https://api.physicscode.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, physicscodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.physicscode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.physicscode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          physicscode: { itemId: "msg_physicscode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              physicscode: { itemId: "msg_physicscode_part" },
    ...
    expect(result[0].providerOptions?.physicscode?.itemId).toBe("msg_physicscode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.physicscode?.itemId).toBe("msg_physicscode_part")
