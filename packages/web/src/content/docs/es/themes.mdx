...
Con PhysicsCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, PhysicsCode usa nuestro propio tema `physicscode`.
...
PhysicsCode viene con varios temas integrados.
...
- Quiere que PhysicsCode coincida con la apariencia de su terminal
...
PhysicsCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/physicscode/themes/*.json` o `$XDG_CONFIG_HOME/physicscode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.physicscode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.physicscode/themes/*.json`
