---
title: LSP Servers
description: PhysicsCode integrates with your LSP servers.
---

PhysicsCode integrates with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

PhysicsCode comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `PHYSICSCODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When physicscode opens a file, it:
...
You can customize LSP servers through the `lsp` section in your physicscode config.
