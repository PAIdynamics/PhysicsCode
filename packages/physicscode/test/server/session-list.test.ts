...
const originalWorkspaces = Flag.PHYSICSCODE_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.PHYSICSCODE_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session.list", () => {
  test("does not filter by directory when directory is omitted", async () => {
    ...
    await mkdir(path.join(tmp.path, "packages", "physicscode"), { recursive: true })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const current = await Instance.provide({
          directory: path.join(tmp.path, "packages", "physicscode"),
  ...
  test("filters by directory when directory is provided", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const ids = (await svc.list({ directory: path.join(tmp.path, "packages", "physicscode") })).map((s) => s.id)
  ...
  test("filters by path and ignores directory when path is provided", async () => {
    Flag.PHYSICSCODE_EXPERIMENTAL_WORKSPACES = false
    await using tmp = await tmpdir({ git: true })
    await mkdir(path.join(tmp.path, "packages", "physicscode", "src", "deep"), { recursive: true })
    await mkdir(path.join(tmp.path, "packages", "app"), { recursive: true })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const current = await Instance.provide({
          directory: path.join(tmp.path, "packages", "physicscode", "src"),
          fn: async () => svc.create({ title: "current" }),
        })
        const deeper = await Instance.provide({
          directory: path.join(tmp.path, "packages", "physicscode", "src", "deep"),
  ...
  test("falls back to directory when filtering legacy sessions without path", async () => {
    Flag.PHYSICSCODE_EXPERIMENTAL_WORKSPACES = false
    await using tmp = await tmpdir({ git: true })
    await mkdir(path.join(tmp.path, "packages", "physicscode", "src"), { recursive: true })
    await mkdir(path.join(tmp.path, "packages", "app"), { recursive: true })
