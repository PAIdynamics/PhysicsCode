...
W PhysicsCode obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
PhysicsCode zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
PhysicsCode ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

PhysicsCode ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `physicscode.json`:
...
- Globalnie: `~/.config/physicscode/agents/`
- W projekcie: `.physicscode/agents/`
...
Jeśli nie zostanie skonfigurowana, PhysicsCode używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji PhysicsCode, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji PhysicsCode w `provider/model-id`. Na przykład, [PhysicsCode Zen](/docs/zen), użyj `physicscode/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `physicscode models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/PAIdynamics/PhysicsCode).
:::
