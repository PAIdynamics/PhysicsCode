...
const MANAGED_PLIST_DOMAIN = "ai.physicscode.managed"

// Keys injected by macOS/MDM into the managed plist that are not PhysicsCode config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/physicscode"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "physicscode")
    default:
      return "/etc/physicscode"
  }
}

export function managedConfigDir() {
  return process.env.PHYSICSCODE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
