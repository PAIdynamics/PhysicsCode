...
import { createOpencode } from "@physicscode-ai/sdk"
...
`)
  ...
  const physicscode = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await physicscode.client.session.create()
    const result = await physicscode.client.session
      .prompt({
