...
Med PhysicsCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger PhysicsCode vårt eget `physicscode`-tema.
...
PhysicsCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/physicscode/themes/*.json` eller `$XDG_CONFIG_HOME/physicscode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.physicscode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.physicscode/themes/*.json`
