---
title: Plugin
description: Scrivi plugin per estendere PhysicsCode.
---

I plugin ti permettono di estendere PhysicsCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di PhysicsCode.
...
- `.physicscode/plugins/` - plugin a livello progetto
- `~/.config/physicscode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/physicscode/node_modules/`.
...
1. Config globale (`~/.config/physicscode/physicscode.json`)
2. Config di progetto (`physicscode.json`)
3. Directory plugin globale (`~/.config/physicscode/plugins/`)
4. Directory plugin di progetto (`.physicscode/plugins/`)
...
PhysicsCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di physicscode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere physicscode.
...
:::note
Se usi l'app desktop di PhysicsCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a physicscode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a physicscode:
...
L'helper `tool` crea uno strumento personalizzato che physicscode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in physicscode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://physicscode.ai/docs/sdk) per i dettagli.
