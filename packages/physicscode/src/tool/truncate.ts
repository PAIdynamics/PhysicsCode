...
export interface Interface {
  ...
  /**
   * Resolved truncation limits: values from `tool_output` in physicscode config, or MAX_LINES / MAX_BYTES if unset.
   */
  readonly limits: () => Effect.Effect<{ maxLines: number; maxBytes: number }>
}

export class Service extends Context.Service<Service, Interface>()("@physicscode/Truncate") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cleanup = Effect.fn("Truncate.cleanup")(function* () {
      // Identifier.timestamp() can't round-trip real-world millisecond
      // timestamps losslessly (timestamp * 4096 + counter needs ~53 bits,
      // but only 48 are stored), so age comparisons based on it are
      // unreliable. Use the file's actual mtime instead.
