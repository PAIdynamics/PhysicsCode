---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w physicscode.
---

import config from "../../../../config.mjs"
export const console = config.console

physicscode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji physicscode.
...
w `~/.local/share/physicscode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim physicscode
config.
...
PhysicsCode Zen to lista modeli dostarczonych przez zespół physicscode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z physicscode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od PhysicsCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `PhysicsCode Zen` i przejdź do [physicscode.ai/auth](https://physicscode.ai/zen).
...
Działa jak każdy inny dostawca w physicscode i jest całkowicie opcjonalny w użyciu.

---

## PhysicsCode Go

PhysicsCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół physicscode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z physicscode.

1. Uruchom polecenie `/connect` w TUI, wybierz `PhysicsCode Go` i przejdź do [physicscode.ai/auth](https://physicscode.ai/zen).
...
Aby używać Amazon Bedrock z physicscode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania physicscode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `physicscode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w physicscode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować physicscode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod physicscode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji physicscode.
...
physicscode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować physicscode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`physicscode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [GitLab authentication helper](https://www.npmjs.com/package/opencode-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem physicscode:
...
Aby używać Google Vertex AI z physicscode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania physicscode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą physicscode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [physicscode-helicone-session](https://github.com/H2Shami/physicscode-helicone-session), aby automatycznie rejestrować każdą konwersację physicscode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja physicscode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować physicscode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować physicscode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować physicscode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem physicscode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/physicscode).
:::
...
Aby korzystać z Ollama Cloud z physicscode:
...
3. Skopiuj klucz API do użycia w physicscode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w physicscode musisz pobrać informacje o modelu lokalnie:
...
PhysicsCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół physicscode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>PhysicsCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **PhysicsCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API physicscode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji physicscode.
...
5. Możesz je także dostosować za pomocą konfiguracji physicscode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z physicscode:
...
Możesz także dostosować modele za pomocą konfiguracji physicscode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z physicscode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `physicscode.json` w katalogu projektu:
...
Pola `limit` pozwalają physicscode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `physicscode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację physicscode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji physicscode.
