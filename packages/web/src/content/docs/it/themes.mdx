...
Con PhysicsCode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, PhysicsCode usa il tema `physicscode`.
...
PhysicsCode include diversi temi integrati.
...
- Vuole che PhysicsCode corrisponda all'aspetto del terminale
...
PhysicsCode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/physicscode/themes/*.json` o `$XDG_CONFIG_HOME/physicscode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.physicscode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.physicscode/themes/*.json`
