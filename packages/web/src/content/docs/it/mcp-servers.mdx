...
Puoi aggiungere strumenti esterni a PhysicsCode usando il _Model Context Protocol_, o MCP. PhysicsCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [PhysicsCode Config](https://physicscode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/physicscode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
PhysicsCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, PhysicsCode:
...
Se il server richiede autenticazione, PhysicsCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `physicscode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, PhysicsCode memorizzera' i token in modo sicuro in `~/.local/share/physicscode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in PhysicsCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione PhysicsCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare PhysicsCode al tuo account Sentry.
