...
Crea `.physicscode/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di PhysicsCode o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di PhysicsCode:
...
- Globale: `~/.config/physicscode/commands/`
- Per progetto: `.physicscode/commands/`
...
physicscode include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
