---
title: Sağlayıcılar
description: physicscode'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

physicscode, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı physicscode yapılandırmanızda yapılandırın.
...
`~/.local/share/physicscode/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları physicscode'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
PhysicsCode Zen, physicscode ekibi tarafından sağlanan modellerin bir listesidir.
physicscode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz PhysicsCode Zen ile başlamanızı öneririz.
:::
...
physicscode'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## PhysicsCode Go

PhysicsCode Go, physicscode ile iyi çalıştığı test edilmiş ve doğrulanmış, physicscode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `PhysicsCode Go`'yu seçin ve [physicscode.ai/auth](https://physicscode.ai/zen) adresine gidin.
...
Amazon Bedrock'u physicscode ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   physicscode'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `physicscode.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi physicscode'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
physicscode'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   physicscode'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
physicscode, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. physicscode'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`physicscode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [GitLab authentication helper](https://www.npmjs.com/package/opencode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`physicscode.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi physicscode'la kullanmak için:
...
Google Vertex AI'yi physicscode ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları physicscode çalıştırılırken ayarlayın.
...
Helicone'un physicscode aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her physicscode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [physicscode-helicone-session](https://github.com/H2Shami/physicscode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her physicscode görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
physicscode'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla physicscode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla physicscode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini physicscode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/physicscode)'a bakın.
:::
...
Ollama Cloud'u physicscode ile kullanmak için:
...
3. physicscode'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: physicscode'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
PhysicsCode Zen, physicscode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>PhysicsCode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **PhysicsCode Zen**'i arayın.

   ```txt
   /connect
   ```

3. physicscode API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   physicscode yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca physicscode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri physicscode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı physicscode ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `physicscode.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları physicscode'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `physicscode auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için physicscode yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, physicscode yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
