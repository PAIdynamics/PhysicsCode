...
Existem dois tipos de agentes no physicscode; agentes primários e subagentes.
...
physicscode vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
physicscode vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

physicscode vem com dois agentes primários integrados e dois subagentes integrados.
...
Configure os agentes em seu arquivo de configuração `physicscode.json`:
...
- Por projeto: `.physicscode/agents/`
...
Se nenhuma temperatura for especificada, o physicscode usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do physicscode quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do physicscode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [PhysicsCode Zen](/docs/zen), você usaria `physicscode/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `physicscode models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/PAIdynamics/PhysicsCode).
:::
