...
PhysicsCode Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

:::note
PhysicsCode Go je trenutno u beta fazi.
:::

Go radi kao bilo koji drugi provajder u PhysicsCode-u. Pretplatite se na PhysicsCode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili PhysicsCode.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa PhysicsCode-om.
:::
...
PhysicsCode Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

PhysicsCode Go radi kao bilo koji drugi provajder u PhysicsCode-u.

1. Prijavite se na **<a href={console}>PhysicsCode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `PhysicsCode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na PhysicsCode Go.
:::
...
PhysicsCode Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj PhysicsCode konfiguraciji
koristi format `physicscode-go/<model-id>`. Na primjer, za Kimi K2.6, koristili biste
`physicscode-go/kimi-k2.6` u svojoj konfiguraciji.
...
Napravili smo PhysicsCode Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz PhysicsCode koristite i bilo kojeg drugog provajdera.
