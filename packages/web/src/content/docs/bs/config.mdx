---
title: Konfiguracija
description: Korištenje PhysicsCode JSON konfiguracije.
---

Možete konfigurirati PhysicsCode koristeći JSON konfiguracijski fajl.

---

## Format

PhysicsCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/physicscode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/physicscode/physicscode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`PHYSICSCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`physicscode.json` u projektu) - postavke specifične za projekat
5. **`.physicscode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`PHYSICSCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.physicscode` i `~/.config/physicscode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/physicscode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu PhysicsCode konfiguraciju u `~/.config/physicscode/physicscode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/physicscode/tui.json`.
...
Dodajte `physicscode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se PhysicsCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `PHYSICSCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `PHYSICSCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.physicscode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.physicscode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`physicscode.ai/config.json`**](https://physicscode.ai/config.json).

TUI konfiguracija koristi [**`physicscode.ai/tui.json`**](https://physicscode.ai/tui.json).
...
Koristite `PHYSICSCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `physicscode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `physicscode serve` i `physicscode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš PhysicsCode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `physicscode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj PhysicsCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, PhysicsCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/physicscode/agents/` ili `.physicscode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, PhysicsCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`physicscode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/physicscode/commands/` ili `.physicscode/commands/`. [Saznajte više ovdje](/docs/commands).
...
PhysicsCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, PhysicsCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju PhysicsCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.physicscode/plugins/` ili `~/.config/physicscode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite PhysicsCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
