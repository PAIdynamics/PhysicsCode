...
- Author package entrypoint is `@physicscode-ai/plugin/tui`.
...
```json
{
  "$schema": "https://physicscode.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/physicscode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@physicscode-ai/plugin/tui`.
- `@physicscode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@physicscode-ai/plugin/tui"
...
- The value is a semver range checked against the running PhysicsCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.physicscode` is absent, no check is performed (backward compatible).
...
- `physicscode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `physicscode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.physicscode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.physicscode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.physicscode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.physicscode` dir or `PHYSICSCODE_CONFIG_DIR`, PhysicsCode installs `@physicscode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@physicscode-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.physicscode/themes` area near the plugin config source.
...
- `--pure` / `PHYSICSCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.physicscode/plugins/tui-smoke.tsx`
- Local vim plugin: `.physicscode/plugins/tui-vim.tsx`
- Local smoke config: `.physicscode/tui.json`
- Local smoke theme: `.physicscode/plugins/smoke-theme.json`
