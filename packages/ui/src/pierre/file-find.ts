...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("physicscode-find")
  api.delete("physicscode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("physicscode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("physicscode-find", new Highlight(...rest))
    return true
  }
