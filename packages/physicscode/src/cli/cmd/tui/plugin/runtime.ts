...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@physicscode-ai/plugin/tui"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".physicscode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".physicscode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".physicscode", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await Instance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.PHYSICSCODE_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.PHYSICSCODE_PURE && config.plugin_origins?.length) {
