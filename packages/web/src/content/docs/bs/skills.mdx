...
Agent skills omogucavaju PhysicsCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
PhysicsCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.physicscode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/physicscode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, PhysicsCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.physicscode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/physicscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.physicscode/skills/git-release/SKILL.md` ovako:
...
PhysicsCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `physicscode.json`:
...
**Za ugradene agente** (u `physicscode.json`):
