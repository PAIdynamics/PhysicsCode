...
Com o physicscode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o physicscode usa nosso próprio tema `physicscode`.
...
O physicscode vem com vários temas embutidos.
...
- Querem que o physicscode corresponda à aparência do seu terminal
...
O physicscode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/physicscode/themes/*.json` ou `$XDG_CONFIG_HOME/physicscode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.physicscode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.physicscode/themes/*.json`
