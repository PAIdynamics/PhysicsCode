...
import { createOpencode, type ToolPart } from "@physicscode-ai/sdk"
...
console.log("🚀 Starting physicscode server...")
const physicscode = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await physicscode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new physicscode session...")
    const { client, server } = physicscode
    ...
    console.log("✅ Created physicscode session:", createResult.data.id)
  ...
  console.log("📝 Sending to physicscode:", message.text)
