...
_Model Bağlam Protokolü_ veya MCP kullanarak physicscode'a harici araçlar ekleyebilirsiniz. physicscode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [physicscode Config](https://physicscode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/physicscode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
physicscode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde physicscode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa physicscode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `physicscode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından physicscode, belirteçleri `~/.local/share/physicscode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra physicscode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi physicscode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve physicscode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
